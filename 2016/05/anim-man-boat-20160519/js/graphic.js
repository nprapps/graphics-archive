...
var render = function(containerWidth) {
    ...
    var sprite = FOLDER + '/filmstrip-' + BREAKPOINTS[BREAKPOINTS.length - 1] + '.jpg';
    for (var i = 0; i < BREAKPOINTS.length; i++) {
        if (i == 0 && containerWidth <= BREAKPOINTS[i]) {
            sprite = FOLDER + '/filmstrip-' + BREAKPOINTS[i] + '.jpg';
    ...
    var canvidControl = canvid({
        ...
        videos: {
            ...
            photo: { src: sprite, frames: 45, cols: 45, fps: 10 }
        },
        width: containerWidth,
            // multiply by height, width of original image
            height: Math.floor(containerWidth * 1080/1920),
        loaded: function() {
