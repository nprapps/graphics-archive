...
var formatData = function() {
    DATA.forEach(function(d) {
        ...
        for (var key in d) {
            if (key == 'label' || key == 'values' || key == 'party' || key == 'requests' || key == 'success') {
                continue;
            }
...
var renderStackedBarChart = function(config) {
     ...
     */
     ...
     var colorScale = d3.scale.ordinal()
         .domain(d3.keys(config['data'][0]).filter(function(d) {
             return d != labelColumn && d != 'values' && d != 'party' && d != 'success' && d != 'full' && d != 'part' && d != 'requests';
         }))
     ...
     */
     ...
     chartElement.append('g')
        ...
        .append('text')
            .text(function(d) {
                return d['success'] + '% (of ' + d['requests'] + ' requests)';
            })
            .attr('class', function(d) {
                return classify(d['label']);
            })
            .attr('x', function(d) {
 				return xScale(d['success']);
            })
            .attr('y', function(d, i) {
