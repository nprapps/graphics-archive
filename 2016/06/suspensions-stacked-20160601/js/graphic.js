...
var formatData = function() {
    DATA.forEach(function(d) {
        ...
        for (var key in d) {
            if (key == 'label' || key == 'values' || key == 'labeltitle') {
                continue;
            }
...
var renderStackedBarChart = function(config) {
    ...
    var labeltitleColumn = 'labeltitle';
    ...
    var chartHeight = ((barHeight + barGap) * config['data'].length) - barGap + 5;
     ...
     */
     ...
     var colorScale = d3.scale.ordinal()
         .domain(d3.keys(config['data'][0]).filter(function(d) {
             return d != labelColumn && d != labeltitleColumn && d != 'values';
         }))
    ...
    // Get rid of the grid since it overlaps the labels
    // chartElement.append('g')
    //     .attr('class', 'x grid')
    //     .attr('transform', makeTranslate(0, chartHeight))
    //     .call(xAxisGrid()
     ...
     group.append('g')
        ...
        .enter().append('text')
            .text(function(d) {
                if (d['val'] != 0) {
                  if (isMobile) {
                      return '$' + d['val'].toFixed(2) + 'B';
                  }
                  return '$' + d['val'].toFixed(2) + ' billion';
                }
            })
            .attr('class', function(d) {
                return classify(d['name']);
    ...
    var labels = chartWrapper.append('ul')
        ...
        .attr('style', formatStyle({
            'width': '100%',
            //'width': labelWidth + 'px',
            'top': margins['top'] - labelTopOffset + 'px',
        ...
        .append('li')
            ...
            .append('span')
                .html(function(d) {
                    return '<strong>' + d[labeltitleColumn] + '</strong> &ndash; ' + d[labelColumn];
                });
}
