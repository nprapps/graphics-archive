...
var formatData = function() {
    DATA.forEach(function(d) {
        ...
        for (var key in d) {
            if (key == 'label' || key == 'values' || key == 'date' || key == 'label_display') {
                continue;
            }
     ...
     */
     ...
     var colorScale = d3.scale.ordinal()
         .domain(d3.keys(config['data'][0]).filter(function(d) {
             return d != labelColumn && d != 'values' && d != 'date' && d != 'label_display';
         }))
         .range([ COLORS['red2'], COLORS['red5'], '#ccc' ]);
     ...
     group.append('g')
        ...
        .enter().append('text')
            .text(function(d,i) {
                ...
                if (val != 0) {
                    if (p.classed('pulse-orlando-nightclub-in-orlando-fla')) {
                        val += ' ' + d['name'].toLowerCase();
                    }
                    return val;
                }
            })
            ...
            .attr('dx', function(d) {
                ...
                if ((textWidth + valueGap * 2 > barWidth) && d['name'] == 'Injured') {
