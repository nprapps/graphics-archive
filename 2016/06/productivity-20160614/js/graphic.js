...
var renderColumnChart = function(config) {
    ...
    var xAxis = d3.svg.axis()
        ...
        .tickFormat(function(d, i) {
            // if ((isMobile && i % 8 == 0) || (!isMobile && i % 4 == 0)) {
            //     return d;
            // }
        });
    ...
    // vertical year lines
    var yearLines = chartElement.append('g')
        .attr('class', 'year-lines');
    var barGap = (xScale(xScale.domain()[1]) - xScale(xScale.domain()[0]) - xScale.rangeBand());
    xScale.domain().forEach(function(d,i) {
        if (i % 4 == 0) {
            yearLines.append('line')
                .attr('x1', xScale(d) - (barGap/2))
                .attr('x2', xScale(d) - (barGap/2))
                .attr('y1', yScale(yScale.domain()[1]))
                .attr('y2', chartHeight + margins['bottom']);

            var yearText = fmtYearFull(config['data'][i]['year']);
            if (isMobile) {
                yearText = '\u2019' + fmtYearAbbrev(config['data'][i]['year']);
            }

            var yearTextPos = xScale(d) - (barGap/2) + ((xScale.rangeBand() + barGap) * 2);
            if (fmtYearFull(config['data'][i]['year']) == '2016') {
                yearTextPos = xScale(d) + (xScale.rangeBand() / 2) - 2;
            }
            ...
            yearLines.append('text')
                ...
                .attr('class', 'y-' + fmtYearFull(config['data'][i]['year']));
        }
    });
