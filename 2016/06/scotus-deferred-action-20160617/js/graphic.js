...
var formatData = function() {
    DATA.forEach(function(d) {
        ...
        for (var key in d) {
            if (key == 'label' || key == 'desc' || key == 'values') {
                continue;
            }
...
var renderStackedBarChart = function(config) {
     ...
     */
     var min = d3.min(config['overallData'], function(d) {
     ...
     var colorScale = d3.scale.ordinal()
         ...
         .range([ COLORS['teal3'], '#999', COLORS['teal3'], '#ccc' ]);
     ...
     */
      ...
      * Render bar values.
      */
    var valueLabels = chartWrapper.append('div')
        ...
        .enter().append('p')
            .html(function(d) {
                if (d['val'] != 0) {
                    return '<strong>' + fmtComma(d['val']) + '</strong> ' + d['name'];
                }
            })
            .attr('style', function(d,i) {
                var s = '';
                s += 'position: absolute; '
                s += 'top: ' + (margins['top'] + 6) + 'px; ';
                if (d['val'] > 1000000) {
                    s += 'left: ' + (margins['left'] + 6 + xScale(d['x0'])) + 'px; ';
                } else {
                    s += 'left: ' + (margins['left'] + 6 + xScale(d['x1'])) + 'px; ';
                }
                return s;
            })
            .attr('class', function(d,i) {
    ...
    /*
     group.append('g')
        ...
        .enter().append('text')
            ...
            .attr('dy', valueGap*2)
            // .call(wrapText, xScale(d['x1']-d['x0']), 12);
    */
