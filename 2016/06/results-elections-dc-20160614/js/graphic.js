var DATA_URL = 'https://elections.npr.org/data/results-2016-06-14.json';
    ...
    parties.forEach(function(d,i) {
        ...
        var precincts = containerElement.select('.precincts')
            .text(function() {
                ...
                if (!p['called'] && p['total'] == 0) {
                    return p['poll_closing'];
                } else {
                    return (p['precinctsreportingpct'] * 100).toFixed(1) + '% of precincts reporting (' + fmtComma(p['precinctsreporting']) + ' of ' + fmtComma(p['precinctstotal']) + ')';
                }
            });
