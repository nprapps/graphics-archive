...
var render = function(containerWidth) {
    ...
    if (containerWidth <= MOBILE_THRESHOLD) {
        ...
        isMobile = false;
        contentWidth = Math.floor((containerWidth - gutterWidth) / 2);
    }

    var textElement = d3.select('.text-content')
        .attr('style', function() {
            if (isMobile) {
                return '';
            } else {
...
var renderColumnChart = function(config) {
    ...
    /*
     * Render lines across bars.
     */

    config['data'].forEach(function(d,i) {
        chartElement.append('line')
            .attr('class', 'y grid grid-pop')
            .attr('x1', xScale(d[labelColumn]))
            .attr('x2', xScale(d[labelColumn]) + xScale.rangeBand())
            .attr('y1', yScale(d['redline']))
            .attr('y2', yScale(d['redline']));
    })

    chartElement.append('line')
        .attr('class', 'y grid side-tick')
        .attr('x1', xScale(['2013']) + xScale.rangeBand())
        .attr('x2', chartWidth)
        .attr('y1', yScale('13.2'))
        .attr('y2', yScale('13.2'))
    ...
    annotations.append('text')
        ...
        .attr('x', chartWidth + 6)
        .attr('y', yScale('13.2') + 3)
        .attr('dy', 0)
        .html(LABELS['annotation'])
