...
var pymChild = null;
var isMobile = false;
var excludedColumns = [ 'label', 'values', 'offset', 'other' ];
...
var formatData = function() {
    DATA.forEach(function(d) {
        ...
        for (var key in d) {
            if (_.contains(excludedColumns, key)) {
                continue;
            }
...
var renderStackedBarChart = function(config) {
     ...
     */
     ...
     var colorScale = d3.scale.ordinal()
         .domain(d3.keys(config['data'][0]).filter(function(d) {
             if (!_.contains(excludedColumns, d)) {
                 return d;
             }
         }))
         .range([ COLORS['teal3'], COLORS['teal5'], '#999', COLORS['orange3'] ]);
    ...
    if (!isMobile) {
        containerElement.select('.key').attr('style', 'margin-left: ' + margins['left'] + 'px;');
    }
