...
var formatData = function() {
    ...
    DATA.forEach(function(d) {
        d['key'] = d['Group'];
        d['values'] = [];
        d['Overall margin of victory'] = +d['Overall margin of victory'];
        d['Estimated youth vote'] = +d['Estimated youth vote'];
        d['Ratio'] = +d['Ratio'];

        _.each(d, function(v, k) {
            if (_.contains(['Group', 'key', 'values', 'Ratio', 'party_win', 'margin_orig', 'show'], k)) {
                return;
            }

            d['values'].push({ 'label': k, 'amt': +v, 'party': d['party_win'] });
            delete d[k];
        });

        delete d['Group'];
    });
    DATA = _.sortBy(DATA, 'Ratio');
    DATA.reverse();
...
var renderGroupedBarChart = function(config) {
    ...
    var barGroups = chartElement.selectAll('.bars')
        ...
        .append('g')
            .attr('class', function(d) {
                return 'g bars ' + classify(d['party_win']);
            })
    ...
    barGroups.selectAll('rect')
        ...
        .append('rect')
            ...
            .attr('y', function(d, i) {
                if (i == 0) {
                    return (barHeight - bulletBarHeight) / 2;
                } else {
                    return 0;
                }
            })
            ...
            .attr('height', function(d, i) {
                if (i == 1) {
                    ...
                    var sibling = parent.select('.estimated-youth-vote');
                    sibling.moveToFront();
                }

                if (i == 0) {
                    return bulletBarHeight;
                } else {
                    return barHeight;
                }
            });
    ...
    barGroups.append('circle')
        .attr('cx', chartWidth + (margins['right'] / 2))
        .attr('cy', (barHeight / 2))
        .attr('r', function(d, i) {
            return radius(d['Ratio']);
        });

    // ratio numbers
    barGroups.append('text')
        .text(function(d) {
            return d['Ratio'].toFixed(1);
        })
    ...
    //     .append('text')
    //         .text(function(d) {
    //             var v = fmtComma(d[valueColumn].toFixed(0));
    //
