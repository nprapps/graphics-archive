...
var formatData = function() {
    DATA.forEach(function(d) {
        ...
        for (var key in d) {
            if (key == 'label' || key == 'values' || key == 'delegates') {
                continue;
            }
...
var renderStackedBarChart = function(config) {
     ...
     */
     ...
     var colorScale = d3.scale.ordinal()
         .domain(d3.keys(config['data'][0]).filter(function(d) {
             return d != labelColumn && d != 'values' && d != 'delegates';
         }))
    ...
    chartElement.append('g')
        ...
        .enter()
            ...
            .text(function(d) {
                var del = +d['delegates'];
                return del.toFixed(0);
            })
            .attr('x', chartWidth + (margins['right'] / 2))
            .attr('y', function(d,i) {
                return ((i * (barHeight + barGap)) + (barHeight / 2) + 5);
            });
