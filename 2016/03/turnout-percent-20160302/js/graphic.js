...
var renderGroupedBarChart = function(config) {
    ...
    var barBgs = chartElement.selectAll('rect.bg')
        .data(config['data'])
            .enter()
        .append('rect')
            .attr('class', function(d,i) {
            ...
            .attr('height', (barHeight * 2) + barGapInner + barGap)
            .attr('transform', function(d,i) {
                var yOffset = -barGap / 2;
                if (i == 0) {
                    return makeTranslate(-margins['left'], yOffset);
                }

                return makeTranslate(-margins['left'], ((groupHeight + barGap) * i) + yOffset);
            });
    ...
    barGroups.append('g')
        ...
        .append('text')
            .text(function(d,i) {
                ...
                if (d[valueColumn] == 0) {
                    if (d[labelColumn] == 'Democrat') {
                        return 'No Democratic contest held';
                    }
                }
                if (d[valueColumn] == 0) {
                    if (d[labelColumn] == 'Republican') {
                        return 'No presidential preference vote held';
                    }
                }
            ...
            .attr('x', function(d) {
                if (isMobile && d[labelColumn] == 'Democrat') {
                    return xScale(0)
                } else {
                    return xScale(d[valueColumn]);
                }
            ...
            .attr('dx', function(d) {
                ...
                // Negative case
                if (!isMobile && d[valueColumn] < 0 || d[valueColumn] == 0 && d[labelColumn] == 'Democrat' && !isMobile) {
                    var outsideOffset = -(valueGap + textWidth);

                    if (d[valueColumn] !== 0 && xStart + outsideOffset < 0) {
                ...
                // Positive case
                    ...
                    if (d[valueColumn] !== 0 && xStart + valueGap + textWidth > chartWidth) {
                        d3.select(this).classed('in', true)
