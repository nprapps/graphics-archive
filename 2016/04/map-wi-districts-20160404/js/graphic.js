...
var CITY_LABEL_ADJUSTMENTS = {
    'Eau Claire': { 'text-anchor': 'middle', 'dy': 14 },
    'Madison': { 'text-anchor': 'end', 'dx': -6, 'dy': 3 },
    'Superior': { 'text-anchor': 'middle', 'dy': -6 },
    'Wausau': { 'text-anchor': 'middle', 'dy': -6 },
}
...
var renderLocatorMap = function(config) {
    ...
    var mapHeight = Math.ceil((config['width'] * aspectHeight) / aspectWidth) + marginBottom;
    ...
    projection = d3.geo.mercator()
        .center(centroid)
        .scale(mapScale)
        .translate([ mapWidth/2, (mapHeight - marginBottom)/2 ]);
        ...
        .enter().append('path')
            .attr('class', function(d) {
                ...
                if (d['id'] == '55') {
                    ...
                    c += ' ' + classify(DISTRICT_LEANINGS[dist]);
                }
                return c;
            })
            .attr('d', path);
    ...
    d3.select('.districts path.s-' + primaryStateClass)
        .moveToFront()
        .classed('primary ' + primaryStateClass, true);
    ...
    // var primaryStateClass = classify(config['primaryState']);
    //
    // d3.select('.country-labels text.' + primaryStateClass)
    ...
    for (city in CITIES) {
        var c = classify(CITIES[city]);
        var cityLabels = d3.selectAll('.' + c);
        cityLabels.classed('active', true);
        if (c == 'madison') {
            cityLabels.classed('capital', true);
        }
    }
    ...
    var scaleBarStart = [0, mapHeight - 20];
