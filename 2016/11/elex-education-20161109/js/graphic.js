...
var renderDotChart = function(config) {
    ...
    var valueColumn = 'coll';
    var valueColumn2 = 'noncoll';
     ...
     */
     ...
     chartElement.append('rect')
        ...
        .attr('y', (6 * (barHeight + barGap)) - (barGap/2))
        .attr('height', chartHeight + barGap - (6 * (barHeight + barGap)) - (barGap/2))
        .attr('fill', '#f1f1f1');
    ...
    _.each(['shadow', 'value'], function(cls) {
        chartElement.append('g')
            ...
            .enter().append('text')
                ...
                .text(function(d, i) {
                    var val = Math.abs(d[valueColumn].toFixed(0));
                    if (i == 0) {
                        if (d[valueColumn] > d[valueColumn2]) {
                            val = val + ' pt. - College';
                        } else {
    ...
    _.each(['shadow', 'value'], function(cls) {
        chartElement.append('g')
            ...
            .enter().append('text')
                .attr('x', function(d, i) {
                    if (d[valueColumn2] > d[valueColumn]) {
                        return xScale(d[valueColumn2]) + 8;
                            ...
                            return xScale(d[valueColumn]) + 14;
                    } else {
                        return xScale(d[valueColumn2]) - 8;
                    }
                })
                ...
                .text(function(d, i) {
                    var val = Math.abs(d[valueColumn2].toFixed(0));
                    if (i == 0) {
                        if (d[valueColumn2] > d[valueColumn]) {
                            val = val + ' pt. - Non-College';
                        } else {
                            val = 'Non-College - ' + val + ' pt.';
                        }
                    }
                    return val;
                });
