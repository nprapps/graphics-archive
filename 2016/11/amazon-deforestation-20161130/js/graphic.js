...
var formatData = function() {
    ...
    keys.forEach(function(c) {
        DATA[c]['data'].forEach(function(d) {
         ...
         */
        var dataSeries = [];
        for (var column in DATA[c]['data'][0]) {
            if (column == 'date') {
                continue;
            }
            ...
            dataSeries.push({
                'name': column,
                'values': DATA[c]['data'].map(function(d) {
                    return {
                        'date': d['date'],
                        'amt': d[column]
                    };
...
var renderLineChart = function(config) {
    ...
    chartElement.append('g')
        ...
        .enter().append('text')
            ...
            .text(function(d) {
                var last = d['values'][d['values'].length - 1];
                var value = last[valueColumn].toFixed(0);
                var year = last[dateColumn];
                var label = fmtYearFull(year) + ': ' + fmtComma(value) + ' sq. miles (estimated)';
                return label;
            })
