...
var $graphic = $('#graphic');
    var graphic_data_url = 'statejobs.csv';
    var graphic_data;
    var bar_height = 17;
    var bar_gap = 6;
    ...
    function drawChart(width) {
        ...
        svg.append('g')
            .selectAll('rect')
                .data(graphic_data)
            .enter().append('rect')
                .attr('x', function(d) { return x(Math.min(0,d.share)) })
                .attr('y', function(d, i) { return i * (bar_height + bar_gap); })
                .attr("width", function(d) { return Math.abs(x(d.share) - x(0)); })
                .attr("height", y.rangeBand())
                // .attr("y", function(d, i) { return i * (bar_height + bar_gap); })
                // .attr("width", function(d){ return x(d.share); })
                .attr("height", bar_height)
                .attr("class", function(d) { return d.share < 0 ? "bar-negative" : "bar-positive"; })
        ...
        svg.append('g')
            ...
            .enter().append('text')
                ...
                .attr('y', function(d, i) { return i * (bar_height + bar_gap); })
                .attr('dx', function(d) { return d.share < 0 ? -4 : 37} )
                .attr('dy', 15)
                .attr('text-anchor', 'end')
                .attr('class', function(d) { return 'l-' + d.state.replace(/\s+/g, '-').toLowerCase() })
...
$(window).load(function() {
            d3.csv(graphic_data_url, function(error, data) {
                ...
                graphic_data.forEach(function(d) {
                    d.state = d.state;
                    d.share = d3.round(+d.share*100,3);
                    d.may = +d.may;
                    d.diff = +d.diff;
                });
