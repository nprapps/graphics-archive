...
function render_bar_chart(width_graphic) {
    ...
    gen_labels.append('text')
    	.attr('x', x(24))
    	.attr('y', function(d) {
    		return (+gen_millennial.attr('y') + 14);
    	})
    	.attr('text-anchor', 'middle')
    	.text('28.7%');

    var gen_boomer = gen_labels.append('text')
    	.attr('class', 'boomer-title')
    	.attr('x', x(60))
    	.attr('y', y(2000000))
    	.attr('text-anchor', 'middle')
    	.text('Boomers');
    ...
    if (Modernizr.svg) {
        ...
        d3.csv(graphic_data_url, function(error, data) {
			...
			graphic_data.forEach(function(d) {
				if (d['age'] >= 14 && d['age'] <= 34) {
					d['generation'] = 'millennial';
				}
