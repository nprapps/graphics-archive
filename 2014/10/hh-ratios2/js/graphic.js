...
var graphic_data_url = 'hh-edu-all-raw.csv';
...
var eduArray = [ "No High School Diploma", "High School Diploma", "Bachelors Degree",]
...
function clickedLevel(d) {
    ...
    for (var id in eduArray) {
        var edID = eduArray[id];
        var cls = classify(eduArray[id]);
        var rows = dataByEd[edID];
    ...
    if (Modernizr.svg) {
        ...
        d3.csv(graphic_data_url, function(error, data) {
            for (var i = 0; i < data.length; i++) {
                ...
                if (!dataByEd.hasOwnProperty(edID)) {
                    dataByEd[edID] = [];
                }

                dataByEd[edID].push(row);
            }
