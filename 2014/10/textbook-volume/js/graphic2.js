...
var graphic_data;
var graphic_data_url = 'bookdata-volume.csv';
...
function draw_graph(width) {
    var color = d3.scale.ordinal()
        .range([ colors['red3'], colors['yellow3'], colors['blue3'], colors['orange3'], colors['teal3'], colors['yellow1'] ]);
    ...
    var lines = svg.append('g')
        ...
        var labelText =  svg.selectAll('label')
            .data(d3.entries(formatted_data))
            .enter()
            .append("text")            
            .attr('class', function(d, i) {
                return 'value-label label-' + classify(d['key']);
            })
            .attr("transform", function(d) {
                var last = d['value'];
             return "translate(" + x(d['value'][last.length-1]['date']) + "," + y(d['value'][last.length-1]['amt']) + ")"; })
            .attr("x",3)
            .attr("dy", ".3em")
            .text(function(d) { 
                return d['key']; 
        ...
        // d3.select('.label-used-books')
        //     .attr('dx','-18em')        
        //     .attr('dy','-6em');
        // d3.select('.label-rented-books')
        //     .attr('dx','-7em')        
        //     .attr('dy','-.2em');
        // d3.select('.label-new-books')
        //     .attr('dx','-18em')        
        //     .attr('dy','-2.7em');
        d3.select('.label-pirated')
            .attr('dy','-.1em');
        d3.select('.label-ebooks')
            .attr('dy','.6em');


        if (is_mobile) {

        d3.select('.label-ebooks-subscription')
            .text('Ebook Subscriptions')
            .attr('dx','-15em')
            .attr('dy','-1.5em');            
