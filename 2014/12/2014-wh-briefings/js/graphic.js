...
var render = function(containerWidth) {
    ...
    graphicWidth = Math.floor((containerWidth - (GRID_MARGIN * (gridIncrement - 1))) / gridIncrement);
    ...
    for (var i = 0; i < keys.length; i++) {
        var leftMargin;
        if (i % gridIncrement == 0) {
            leftMargin = 0;
        } else {
            leftMargin = GRID_MARGIN;
        }
        drawGraph(graphicWidth, keys[i], leftMargin);
    }
 ...
 * DRAW THE GRAPH
 */
var drawGraph = function(graphicWidth, id, marginLeft) {
    var aspectHeight = 3;
    var aspectWidth = 4;
    var color = d3.scale.ordinal()
        .range([ colors['teal1'], colors['teal2'], colors['teal3'], colors['teal4'], colors['teal5'], colors['teal6'] ]);
...
//            return d['amt'].length > 0;
    ...
    var chart = graph.append('div')
        ...
        .attr('style', function() {
            ...
            s += 'margin-left: ' + marginLeft + 'px;';
            return s;
        });

    // draw the legend
    var legend = chart.append('ul')
		.attr('class', 'key')
		.selectAll('g')
