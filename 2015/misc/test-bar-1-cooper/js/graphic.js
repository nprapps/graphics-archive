...
var BAR_GAP = 5;
var GRAPHIC_DATA = [
    { 'label': 'Top-tier private university', 'school': 'Carnegie Mellon', 'amt': 255288 },
    { 'label': 'Public university', 'school': 'average in-state', 'amt': 75772 },
    { 'label': 'For-profit tech school', 'school': 'DeVry', 'amt': 52078 },
    { 'label': 'Coder boot camp', 'school': 'Dev Bootcamp*', 'amt': 12200 },
    { 'label': 'Self-taught', 'amt': 0 }
];
...
var drawGraph = function(graphicWidth) {
    ...
    var x = d3.scale.linear()
        .domain([ 0, d3.max(GRAPHIC_DATA, function(d) { 
            return Math.ceil(d['amt']/50000) * 50000; // round to next 50000
        }) ])
        .range([0, width]);
    ...
    svg.append('g')
        ...
        .enter().append('text')
            ...
            .text(function(d) {
                if (d['label'] == 'Self-taught') {
                    return '$' + fmtComma(d['amt'].toFixed(0)) + ' (for a library card)';
                } else {
                    return '$' + fmtComma(d['amt'].toFixed(0));
                }
            });
