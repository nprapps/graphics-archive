...
var GROUP_GAP = 5;
var BAR_HEIGHT = 4;
var GRAPHIC_DATA_URL = 'strict.json';
...
var drawGraph = function(graphicWidth) {
    ...
    var x = d3.scale.linear()
        .domain([ 0, d3.max(graphicData, function(d) {
            return Math.ceil(d['value']['female'] / 50) * 50; // round to next 50
        }) ])
        .range([0, width]);
    ...
    svg.append('g')
        ...
        .enter().append('rect')
            ...
            .attr('width', function(d){
                return x(d['value']['female']) - x(d['value']['male']);
            })
...
var sortValues = function(a,b) {
    if (a['value']['female'] > b['value']['female']) {
        return -1;
    }
    if (a['value']['female'] < b['value']['female']) {
        return 1;
