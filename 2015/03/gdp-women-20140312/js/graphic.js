...
var onWindowLoaded = function() {
    if (Modernizr.svg) {
        ...
        GRAPHIC_DATA = GRAPHIC_DATA.filter(function(d) {
            return (d['year'] >= 2005) && (d['loss'] >= 20);
        });

        colorD3 = d3.scale.threshold()
            .domain([20, 25, 30, 35]) // bins
            .range([ colors['teal6'], colors['teal5'], colors['teal3'], colors['teal1'] ]); // color palette
...
var drawGraph = function(graphicWidth) {
    ...
    var x = d3.scale.linear()
        .domain([ 0, d3.max(GRAPHIC_DATA, function(d) {
            return Math.ceil(d['loss']/10) * 10; // round to next 10
        }) ])
        .range([0, width]);
