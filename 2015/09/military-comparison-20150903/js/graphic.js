    ...
    var xAxis = d3.svg.axis()
        ...
        .tickFormat(function(d) {
            if (d == 0) {
                ...
                return Math.floor(d / 1000000) + ' million';
            }
        });
...
//         .attr('y2', chartHeight);
    ...
    var spend = chartWrapper.select('svg').append('g')
        .attr('class', 'spending')
        .attr('transform', 'translate(' + (margins['left'] + chartWidth) + ',' + margins['top'] + ')');
    
    var spendText = spend.append('g')
        .attr('class', 'spend-labels')
        .selectAll('text')
            .data(config['data'])
        .enter()
            .append('text')
            .text(function(d) {
                var val = d['spending_gdp'];
                if (val == null) {
                    return 'n/a';
                } else {
                    return val + '%';
