...
//    var chartHeight = Math.ceil((config['width'] * aspectHeight) / aspectWidth) - margins['top'] - margins['bottom'];
    var chartHeight = 300 - margins['top'] - margins['bottom'];
...
//         .call(xAxis);
    ...
    chartElement.append('g')
        ...
        .append('text')
            ...
            .attr('dx', -6)
            .attr('dy', function(d) {
                return adjustLabels(d['label'], 'left');
            })
            .attr('class', function(d) {
                return classify(d[labelColumn]);
            })
    ...
    chartElement.append('g')
        ...
        .append('text')
            ...
            .attr('dx', 6)
            .attr('dy', function(d) {
                return adjustLabels(d['label'], 'right');
            })
            .attr('class', function(d) {
                return classify(d[labelColumn]);
            })
...
// adjust labels
var adjustLabels = function(fuel, side) {
    var dy = 3;

    if (labelPos && labelPos[side]) {
        if (labelPos[side][fuel] != undefined) {
            dy = labelPos[side][fuel];
        }
    }
    return dy;
}
