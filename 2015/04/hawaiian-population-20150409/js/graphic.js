...
var drawGraph = function(graphicWidth) {
    ...
    var color = d3.scale.ordinal()
        .range([ colors['yellow3'], colors['yellow2'], colors['yellow3'] ]);
...
// filter out empty data. uncomment this if you have inconsistent data.
    ...
    y.domain([ 0, d3.max(d3.entries(formattedData), function(c) {
            return d3.max(c['value'], function(v) {
                var n = v['amt'];
                return Math.ceil(n/200000) * 200000; // round to next 200K
            });
        })
    ]);
    ...
    svg.append('rect')
        .attr('class', 'projected area')
        .attr('x', x(d3.time.format('%Y').parse('2010')))
        .attr('y', 0)
        .attr('height', height)
        .attr('width', x(d3.time.format('%Y').parse('2060')) - x(d3.time.format('%Y').parse('2010')))
    ...
    // add label
    svg.append('text')
        .attr('class', 'label-projection')
        .attr('x', x(d3.time.format('%Y').parse('2060')))
        .attr('y', y(formattedData['Future projection'][10]['amt']))
        .attr('dx', -5)
        .attr('dy', -5)
        .text('Projection');
}
