...
var onWindowLoaded = function() {
    if (Modernizr.svg) {
        ...
        GRAPHIC_DATA.forEach(function(d) {
            d['amt'] = +d['amt'];
            d['margin_start'] = +d['margin_start'];
            d['margin_end'] = +d['margin_end'];
        });
...
var drawGraph = function(graphicWidth) {
    ...
    svg.append('g')
        ...
        .enter().append('rect')
            .attr('x', function(d) {
                return x(d['margin_start']);
            })
            .attr('y', function(d, i) {
                return (i * (BAR_HEIGHT + BAR_GAP)) + ((BAR_HEIGHT - ERROR_BAR_HEIGHT) / 2);
            })
            .attr('width', function(d){
                return x(d['margin_end']) - x(d['margin_start']);
            })
            .attr('height', ERROR_BAR_HEIGHT)
            .attr('fill', function(d) {
                return '#dbebeb';
            })
            .attr('class', function(d, i) {
                return 'bar-' + i + ' ' + classify(d['city']);
            });
...
//            .attr('dx', DOT_RADIUS + 6)
            .attr('dy', (BAR_HEIGHT / 2) - 10)
            .attr('text-anchor', 'middle')
            .attr('class', function(d) {
                return classify(d['city']);
            })
            .text(function(d) {
                return d['amt'].toFixed(1) + '%';
            });

    // draw labels for each city
    svg.append('g')
        .attr('class', 'labels')
        .selectAll('text')
            .data(GRAPHIC_DATA)
