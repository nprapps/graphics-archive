...
var drawGraph = function(graphicWidth) {
    ...
    var x = d3.scale.linear()
        .domain([ 0, d3.max(GRAPHIC_DATA, function(d) {
            return Math.ceil(d['debt_pct_gdp']/10) * 10; // round to next 10
        }) ])
        .range([0, width]);
    ...
    svg.append('g')
        ...
        .enter().append('rect')
            ...
            .attr('width', function(d){
                return x(d['debt_pct_gdp']);
            })
            .attr('height', BAR_HEIGHT)
            .attr('fill', function(d) {
                if (d['label'] == 'Puerto Rico') {
                    return COLORS['teal3'];
                } else {
                    return COLORS['teal5'];
                }
    ...
    svg.append('g')
        ...
        .enter().append('text')
            ...
            .attr('dx', function(d) {
                if (x(d['debt_pct_gdp']) > VALUE_MIN_WIDTH) {
                    return -6;
                } else {
                    return 6;
                }
            ...
            .text(function(d) {
                return d['debt_pct_gdp'].toFixed(0) + '%';
            });
