    ...
    if (Modernizr.svg) {
        ...
        d3.csv(GRAPHIC_DATA_URL, function(error, data) {
            ...
            colorD3 = d3.scale.ordinal()
                .range([ COLORS['teal1'], COLORS['teal3'], '#ccc' ])
...
var drawGraph = function(graphicWidth) {
    ...
    group.append('g')
        ...
        .enter().append('text')
            .attr('x', function(d, i) {
				return d['val'] < 15 && isMobile ? x(d['x0']) : x(d['x1']);
            })
            .attr('dx', function(d, i) {
				return d['val'] < 15 && isMobile ? 2 : -2;
            })
            .attr('dy', (BAR_HEIGHT / 2) + 4)
            .attr('text-anchor', function(d, i) {
				return d['val'] < 15 && isMobile ? 'beginning' : 'end';
            })
            .attr('class', function(d) {
                return classify(d['name']);
            })
            .text(function(d) {
                if (d['val'] > 0 && d['name'] !== 'Don\'t Know') {
                    var v = d['val'] + '%';
                    return v;
                }
            });
