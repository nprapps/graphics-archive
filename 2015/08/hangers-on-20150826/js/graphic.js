...
var formatData = function() {
    ...
    candidates = _.pluck(graphicData, 'candidate');
    candidates = d3.set(candidates).values();
}
...
var render = function(containerWidth) {
    ...
    if (containerWidth <= MOBILE_THRESHOLD) {
        ...
        isMobile = false;
        graphicWidth = Math.floor((containerWidth - ((numAcross - 1) * gutterWidth)) / numAcross);
    }
    ...
    candidates.forEach(function(d, i) {
        var candidateData = graphicData.filter(function(c) {
            return c['candidate'] == d;
        });
...
var renderColumnChart = function(config) {
    ...
    var xAxis = d3.svg.axis()
        .scale(xScale)
        .orient('bottom')
        .tickValues([ xScale.domain()[0], xScale.domain()[xScale.domain().length - 1] ]);
     ...
     chartElement.append('g')
         ...
         .append('rect')
             ...
             .attr('class', function(d, i) {
                 return 'bar bar-' + i + ' ' + classify(config['data'][0]['party']);
             });
