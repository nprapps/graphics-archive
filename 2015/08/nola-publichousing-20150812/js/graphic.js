...
var formatData = function(data) {
    data.forEach(function(d) {
        ...
        for (var key in d) {
            if (key == 'data_label' || key == 'label' || key == 'values' || key == 'total') {
                continue;
            }
...
var render = function(containerWidth) {
    ...
    // Render the charts!
    _.each(graphicData, function(data, slug) {
        var multiplier = (isMobile) ? 1 : 0.49;
        renderStackedColumnChart({
            container: '#' + slug + ' .graphic',
...
var renderStackedColumnChart = function(config) {
    ...
    var colorScale = d3.scale.ordinal()
        ...
        .range([ COLORS['teal4'], COLORS['teal5'] ]);
