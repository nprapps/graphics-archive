 ...
 */
var formatDataBullet = function(data_obj) {
...
var render = function(containerWidth) {
    ...
    renderGroupedBarChart({
        container: '#grouped-bar-chart-support',
...
var renderGroupedBarChart = function(config) {
    ...
    var barGapInner = 2;
    var barGap = 10;
    var groupHeight =  (barHeight) + (barGapInner)
    var labelWidth = isMobile ? 85 : 120;
    var labelMargin = 6;
    var valueGap = 6;
    ...
    var chartHeight = ((((barHeight + barGapInner)) + barGap) * numGroups) - barGap + barGapInner;
    ...
    barGroups.append('g')
        ...
        .append('text')
            .text(function(d,i) {
                var v0 = fmtComma(d['values'][0][valueColumn].toFixed(0));
                var v1 = fmtComma(d['values'][1][valueColumn].toFixed(0));

                if (i == 0) {
                    var label_text = '$' + v1 + ' in support (of $' + v0 + ' income)';
                    return label_text;
                } else if (i == 1) {
                    var label_text = '$' + v1 + ' (of $' + v0 + ' monthly income)';
                    return label_text;
                } else {
                    var label_text = '$' + v1 + ' (of $' + v0 + ')';
                    return label_text;
                }
            })
            .attr('x', function(d) {
                if (d['values'][0][valueColumn] < d['values'][1][valueColumn]) {
                    return xScale(d['values'][1][valueColumn]);
                } else {
                    return xScale(d['values'][0][valueColumn]);
                }
            })
            .attr('y', function(d, i) {
                return 0;
            ...
            .attr('dx', function(d) {
                var value_column = d['values'][0][valueColumn];
