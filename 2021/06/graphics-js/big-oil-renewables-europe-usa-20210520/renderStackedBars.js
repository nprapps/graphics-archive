...
module.exports = function(config) {
  ...
  var categories = config.data[0].values.map(d => d[nameColumn]);
  categories.push('Concept');
  var colorScale = d3
    .scaleOrdinal()
    .domain(categories)
    .range([COLORS.teal2, COLORS.teal4, "#ccc", "#e3e3e3" ]);
  ...
  var xAxis = d3
    ...
    .scale(xScale)
    .ticks(ticksX)
    .tickFormat(d => (d == 15 || isMobile.matches && d == 10) ? d + ' gigawatts' : d);
  ...
  group
    ...
    .enter()
    .append("text")
    .text(d => (d.val ? parseFloat(d.val.toFixed(2)) : null))
