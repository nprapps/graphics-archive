...
var renderBarChart = function(config) {
  ...
  var floors = config.data.map(
    d => Math.floor(d.amt / roundTicksFactor) * roundTicksFactor
  );
  ...
  var ceilings = config.data.map(
    d => Math.ceil((d.amt + d.moe) / roundTicksFactor) * roundTicksFactor
  );
  ...
  chartElement
    ...
    .enter()
    .append("rect")
    .attr("x", d => (d.amt >= 0 ? xScale(0) : xScale(d.amt)))
    .attr("width", d => Math.abs(xScale(0) - xScale(d.amt + d.moe)))
