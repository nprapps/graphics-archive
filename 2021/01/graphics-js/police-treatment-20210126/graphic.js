...
var renderStackedBarChart = function(config, num) {
  // Setup
  ...
  var buffer = 30;
  if (isMobile.matches) buffer = 10;
  // Calculate actual chart dimensions
  var chartWidth = (config.width - margins.left - margins.right - buffer)/2;
  ...
  if (config.title && config.showTitle) {
   var header = containerElement.append('h3')
            .text(config.title);
     if (num == 0) header.attr('style', 'padding-left:' + (labelWidth) + 'px;');
  ...
  var leftMarg = 0;
  if ((isMobile.matches && num == 2) || num == 0) leftMarg = margins.left;
  var chartElement = chartWrapper
    .append("svg")
    .attr("width", chartWidth + leftMarg + margins.right)
    .attr("height", chartHeight + margins.top + margins.bottom)
    .append("g")
    .attr("transform", `translate(${leftMarg},${margins.top})`);
  ...
  group
    ...
    .text(function(d) {
      var units =  "%";
      return d.val ? Number(d.val).toFixed(0) + units : null;
    })
    .attr("class", d => classify(d.name))
  ...
  if ((num == 0) || (num == 2 && isMobile.matches)) {
  // Render bar labels.
  chartWrapper
    .append("ul")
    .attr("class", "labels")
