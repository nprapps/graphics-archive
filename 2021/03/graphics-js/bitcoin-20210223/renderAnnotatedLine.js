...
module.exports = function(config) {
   ...
   * Render milestones
   */
  if(config.milestoneDates) {
    var milestoneYPos = chartHeight + 35;

    var milestones = chartElement.append("g")
      .attr("class", "dot-milestones")
      .selectAll("circle")
      .data(config.milestoneDates)
      .enter();

    milestones.append("line")
      .attr("x1", d => xScale(d[dateColumn]))
      .attr("x2", d => xScale(d[dateColumn]))
      .attr("y1", d => milestoneYPos)
      .attr("y2", d => yScale(d[valueColumn]));

    milestones.append("circle")
      .attr("class", "dots")
      .attr("cx", d => xScale(d[dateColumn]))
      .attr("cy", milestoneYPos)
      .attr("r", 8);

    milestones.append("text")
      .attr("x", d => xScale(d[dateColumn]))
      .attr("y", milestoneYPos)
      .attr("dy", 4)
      .text((d,i) => (i + 1));
  }
