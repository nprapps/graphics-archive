...
var { makeTranslate, wrapText } = require("./lib/helpers");
var d3 = {
...
module.exports = function(config) {
  ...
  var clusters = d3.scalePoint()
    .domain(["2", "1", "-1"])
    .range([rangeWidth, chartHeight - rangeWidth])

  var axis = d3.axisLeft(clusters)
    .ticks(numClusters)
  ...
  var gAxis = d3.select(svg)
    .append("g")
    .attr("transform", makeTranslate(100, 0))
  ...
  gAxis
    .append("g")
    .attr("class", "y grid")
    .attr("transform", makeTranslate(80, 0))
  ...
  d3.select(svg)
    ...
    .append("line")
    .attr("x1", 10)
    .attr("y1", chartHeight / 1.5)
    .attr("x2", chartWidth)
    .attr("y2", chartHeight / 1.5)
    .attr("stroke", "#ccc")
    .attr("stroke-width", 2)
  ...
  var simulation = d3.forceSimulation()
    ...
    .force("center", d3.forceCenter(chartWidth / 1.5, chartHeight / 1.5 ));
