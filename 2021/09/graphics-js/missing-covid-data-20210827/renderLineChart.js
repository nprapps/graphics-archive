...
var { dateFull, yearAbbrev } = require("./lib/helpers/formatDate");
...
module.exports = function(config) {
  ...
  var ticksX = d3.timeMonth.every(3);
  var ticksY = 5;
  var roundTicksFactor = 10;

  if (isMobile.matches) {
    ticksX = d3.timeMonth.every(6);
  }
  ...
  var max = 100;//Math.max.apply(null, ceilings);
  ...
  var xAxis = d3
    ...
    .tickFormat(d => [
      d.getMonth() + 1,
      //d.getDate(), 
      d.getFullYear().toString().slice(isMobile.matches ? 0 : 2)
    ].join("/"));
  ...
  chartElement
    ...
    .text(function(d) {
      ...
      var label = value.toFixed(1).replace(/\.0+/, "") + "%";
