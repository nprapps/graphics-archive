...
module.exports = function (config) {
  ...
  var legendData = ['Overall workforce']
  ...
  legend.append("b").style("background-color", d => d == "Overall workforce" ?  '#777' : COLORS.teal3);
  ...
  config.data.forEach(function (chart, index) {
    // margins.left = 0;
    margins.left = 30;
    if (chart.name == "Overall workforce") return;
    var chartWidth =
    ...
    chartElement
      ...
      .attr("class", "lines")
      .selectAll("path")
      .data(config.data.filter(d => ["Overall workforce", chart.name].includes(d.name)))
      .enter()
      .append("path")
    ...
    chartElement
      ...
      .text(function (d) {
        ...
        if (d.name == "Retail store jobs") return label + ' of workers in retail store jobs quit their job in July 2021';
        if (d.name == "Overall workforce" && !isMobile.matches) return 'Overall workforce: ' +  label;
        return label;
