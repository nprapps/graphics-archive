...
var formatData = function () {
  if (!LABELS.show_territories) {
    ...
    DATA_2011 = DATA_2011.filter(d => territories.indexOf(d.state_name) == -1);
  }
};
...
var render = function () {
  ...
  var width = element.offsetWidth / (isMobile.matches ? 1 : 2);
  renderStateGridMap({
    container,
    width,
    data: DATA,
...
var renderStateGridMap = function (config) {
  ...
  var template2011 = d3.select("#map-template-2011");
  containerElement.html(template2011.html() + template.html());
  ...
  for (var i = 0; i < 2; i++) {
    ...
    var data = i == 0 ? config.data_2011 : config.data;
    var id = i == 0 ? '#year-2011' : '#year-2020';
    var chartElement = containerElement.select(id);
