...
module.exports = function (config) {
  ...
  var max = isMobile.matches ? Math.max.apply(null, ceilings) : 20;
  ...
  if (annotated) {
    var annotation = chartElement
      ...
      .selectAll("circle")
      // .data(config.annotations.filter(d => d.series === "retail_real"))
      // .data(config.annotations.filter(d => d.series === "retail_real" || d.series === "grocery_real"))
      .data(config.annotations)
      .enter();

    console.log(config.annotations);

    ["retail_real", "grocery_real"].forEach((item, i) => {
      annotation
        .append("circle")
        .attr("class", "dot")
    ...
    var textContent = function (d) {
      ...
      // return text + " " + value;
      return `${monthYear(d[dateColumn])} ${text}`;
    };
    ...
    annotation
      ...
      .attr("y1", d => yScale(d["retail_real"]) + 6)
      .attr("x2", d => xScale(d[dateColumn]))
      .attr(
        "y2",
        d => yScale(d["grocery_real"]) + yOffset(d) + annotationYOffset - 12
      );
    ...
    annotation
      ...
      .attr("y", d => yScale(d["grocery_real"]) + yOffset(d) + annotationYOffset)
      ...
      var label = enter
        ...
        .attr("transform", function(d) {
          ...
          return makeTranslate(
            ...
            (chartWidth + 5),
            yScale(lastItem(d)[valueColumn]) + yOffset
          )
        });
      label
        .append("tspan")
