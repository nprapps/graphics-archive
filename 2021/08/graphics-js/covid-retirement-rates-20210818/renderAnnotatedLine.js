...
var getAPMonth = require("./lib/helpers/getAPMonth");
...
module.exports = function (config) {
  ...
  var colorScale = d3
    ...
    .range([COLORS.blue2, COLORS.blue4]);
  ...
  // legend.append("label").html(d => config.labels[d]);
  ...
  var yAxis = d3
    ...
    .tickFormat(function (d) {
      return isMobile.matches
        ? `${Math.floor(d / 1000000)}m`
        : `${Math.floor(d / 1000000)} million`;
    });
  ...
  var covid = chartElement.append("g").attr("class", "covid");

  var covidStart = new Date(2020, 1, 1);

  covid
    .attr("transform", makeTranslate(xScale(covidStart), 0))
    .append("line")
    .attr("x1", 0)
    .attr("x2", 0)
    .attr("y1", 0)
  ...
  covid
    .append("text")
    .attr("y", (chartHeight / 3) * 2)
    .attr("class", "shadow")
    .text(config.labels.label_covid)
    .clone()
  ...
  lines.select(".real").raise();

  lines
    .append("circle")
    .attr("cx", xScale(covidStart))
    .attr("cy", yScale(config.data[0].values[61].amt))
  ...
  var labelProjected = labels
    .select("text.projected")
    .attr("transform", d =>
      makeTranslate(
        xScale(lastItem(d)[dateColumn]) + (isMobile.matches ? 5 : 8),
        yScale(lastItem(d)[valueColumn]) + (isMobile.matches ? 3 : 0)
      )
    )
    .append("tspan")
    .attr("x", 0)
  ...
  var labelReal = labels
    .select("text.real")
    .attr("transform", d =>
      makeTranslate(
        xScale(lastItem(d)[dateColumn]) + (isMobile.matches ? 5 : 8),
        yScale(lastItem(d)[valueColumn]) - (isMobile.matches ? 2 : 12)
      )
    )
    .append("tspan")
    .attr("x", 0)
    .text(`${config.labels.label_month}:`)
    .clone()
    .attr("y", "1.2em")
    .text(d => fmtComma(lastItem(d)[valueColumn]));
  ...
  if (!isMobile.matches) {
    labelProjected
      .clone()
      .attr("y", "1.2em")
      .text(config.labels.projected_annot);
    labelReal.clone().attr("y", "2.4em").text(config.labels.real_annot);
  }
};
