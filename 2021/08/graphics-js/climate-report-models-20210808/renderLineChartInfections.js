...
module.exports = function (config) {
  ...
  var annoLen = config.annotations.length - 1;
  ...
  var yScale = d3.scaleLinear().domain([-1, 2]).range([chartHeight, 0]);

  var legendData = config.data.map(d => d.name).slice(0, 1);
  ...
  var lastItem = d => d.values.filter(v => v[valueColumn])[d.values.filter(v => v[valueColumn]).length - 1];
  ...
  chartElement
    ...
    .call(wrapText, isMobile.matches? 50 : 98, 14);
