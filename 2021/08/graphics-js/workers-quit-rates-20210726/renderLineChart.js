...
module.exports = function (config) {
  ...
  var legendData = ['Overall', 'Industry']
  ...
  config.data.forEach(function (chart, index) {
    // margins.left = 0;
    if (index == 1 || index == 3) margins.left = 30;
    if (chart.name == "Overall") return;
    var chartWidth =
      config.width / (isMobile.matches ? 1 : 2) - margins.left - margins.right;
    var chartHeight =
      Math.ceil((config.width * aspectHeight) / aspectWidth) -
      margins.top -
      margins.bottom;
    ...
    chartElement
      .append("g")
      .append("text")
      .attr("x", xScale(new Date(2020, 8, 1)))
    ...
    chartElement
      ...
      .attr("y", function (d) {
        if (d.name == "Restaurant, bars & hotel")
      ...
      .attr("text-anchor", d => (d.name == "Restaurant, bars & hotel") ? "end" : "start")
      .text(function (d) {
        console.log(d)
        var item = lastItem(d);
        ...
        if (d.name == "Restaurant, bars & hotel") return label + ' of workers in restaurant, bars & hotel jobs quit their job in June, 2021'
