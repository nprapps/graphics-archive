...
module.exports = function (config) {
  ...
  var yScale = d3.scaleLinear().domain([min, 1200000]).range([chartHeight, 0]);
  ...
  labelVals
    ...
    .append("text")
    .attr("x", d => xScale(lastItem(d)[dateColumn]) + 8)
    .attr("y", d => yScale(lastItem(d)[valueColumn]) + (isMobile.matches ? 14 : 18) + (d.name.includes("Reported") ? 0: -30))
    .attr("fill", d => colorScale(d.name))
    .text(function (d, i) {
      var item = lastItem(d);
      var value = item[valueColumn];
