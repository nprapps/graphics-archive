...
module.exports = function(config) {
  ...
  var barHeight = 30;
  var barGap = 5;
  var labelWidth = isMobile.matches? 110 : 180;
  var labelMargin = 6;
  var valueGap = 6;
  ...
  var nth= 0;
  var divisor = Math.max(...values);
  while (divisor >= 10) {
    divisor = divisor/10;
    nth += 1;
  }
  var roundTicksFactor = Math.pow(10, nth);
  ...
  var colorScale = d3
    ...
    .domain(config.data[0].values.map(d => d[nameColumn]))
      .range([COLORS.teal3, '#999', COLORS.red2, "#ccc"]);

  var legenddomain = ['Eligible', 'No aid (denied/pending/no decision)']
  // Render the legend.
  ...
  group
    ...
    .append("text")
    .text(function(d) {
        return d.val ? fmtComma(d.val) : null;
    })
    ...
    .attr("dx", function(d, i) {
      ...
      if (textWidth + valueGap * 1.1 > barWidth) {

        if (d.name == "Destroyed" || d.name == "No aid (denied/pending/no decision)") {
