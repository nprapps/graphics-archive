...
var COUNTRY_LABEL_ADJUSTMENTS = {
    'Canada': { 'text-anchor': 'end', 'dx': -150, 'dy': -50 },
    'United States': { 'text-anchor': 'end', 'dx': 0, 'dy': 30 }
}
...
var renderLocatorMap = function(config) {
     ...
     */
    var centroid = [(((bbox[0] + bbox[2]) / 2)+100), (((bbox[1] + bbox[3]) / 2)-5)];
    ...
    projection = d3.geo.conicEqualArea()
        ...
        .rotate([120,0,0])
        //.translate([-170,240])
        .translate([ ((mapWidth/2.7)), mapHeight/1.3 ]);
    ...
    chartElement.append('g')
        .attr('class', 'pipeline')
        .selectAll('path')
            .data(mapData['pipeline']['features'])
        .enter().append('path')
          .attr('d', path);

      /*
      * Render Prudhoe Bay Oil Field.
      */
      // chartElement.append('g')
      //     .attr('class', 'oil-field')
      //     .selectAll('path')
      //       .data(mapData['oil-field']['features'])
      //       .enter().append('path')
      //         .attr('d', path);
