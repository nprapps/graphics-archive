...
/**
 * Get a single item d3 selection from another selection, by index.
 *
 * @param {d3.selection} selection - D3 selection
 * @param {number} i - Index of the item to return in the new selection.
 *
 * @returns {d3.selection} D3 selection containing the ith item from
 *   the original selection.
 */
var selectByIndex = function(selection, i) {
    return d3.select(selection[0][i]);
};
...
var renderBarChart = function(config) {
    ...
    // For a given draggable bar's index, position the corresponding handle.
    // This is designed to passed to `selection.each()` or a similar
    // method.
    var showDragHandle = function (d, i) {
        var correspondingHandle = selectByIndex(dragHandles, i);
        ...
        if (dragIcon) {
            ...
            dragIcon
                ...
                .attr('transform', 'translate(' + xScale(d['amt']) + ', ' + (barGap + (barHeight/2) - (dragIconBBox.height/2)) + ')');
    ...
    config['dispatch'].on('onscreen', function () {
        ...
        draggableBar.call(animateBars, xScale, valueColumn, function(d, i) {
            showDragHandle(d, i);
            valueGroup.select('.draggable')
                .call(updateValues, xScale, valueColumn, valueGap, chartWidth);
