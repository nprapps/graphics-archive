...
var render = function(containerWidth) {
    ...
    renderStackedColumnChart({
        container: '#stacked-column-chart-seoul',
...
var renderStackedColumnChart = function(config) {
    ...
    var jan_start = d3.time.format('%m/%d/%y').parse('1/1/17'),
        jan_end = d3.time.format('%m/%d/%y').parse('1/1/17'),
        apr_start = d3.time.format('%m/%d/%y').parse('12/1/17'),
        apr_end = d3.time.format('%m/%d/%y').parse('12/1/17');
    ...
    annos_g.append('line')
        ...
        .attr('x1', chartWidth + bar_w + 2)
        .attr('x2', chartWidth + bar_w + 10)
        .attr('y1', yScale(last_item['la']) - 1)
        .attr('y2', yScale(last_item['la']) - 1)

    annos_g.append('text')
        .attr('class', 'anno-label-la anno-avg-la')
        .attr('x', chartWidth + bar_w + 12)
        .attr('y', yScale(last_item['la']) + 5)
        .text('LA Average')
        .call(wrapText, config['width'] - chartWidth - margins['left'] - 15, 12)  */;
}
