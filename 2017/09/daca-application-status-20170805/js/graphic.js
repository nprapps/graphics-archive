...
var renderStackedColumnChart = function(config) {
    ...
    bars.selectAll('text')
        ...
        .enter().append('text')
            .text(function(d) {
                if (isMobile) {
                    return Math.round(d['val'] / 1000) + 'K';
                } else {
            ...
            .attr('y', function(d) {
                ...
                if (d['val'] < 2000 && d['val'] > 0) {
                    d3.select(this).classed('outside', true);
                    return (barCenter + textHeight / 2) - textHeight;
                } else if ((textHeight + valueGap * 2 > barHeight) && (d['val'] > 0)) {
                    return (barCenter + textHeight / 2) - (textHeight / 5);
