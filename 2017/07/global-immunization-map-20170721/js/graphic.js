...
var formatData = function() {
    DATA.forEach(function(d) {
        var id = d['id'];
        if (d['number_unvacc'] != null) {
            d['number_unvacc'] = +d['number_unvacc'];
        }
        if (d['lat'] != null) {
            d['lat'] = +d['lat'];
        }
    ...
    if (DATA_DISPLAY == 'bubble') {
        ...
        if (!isMobile) {
            bubbles.on('mouseover', function(d) {
                    ...
                    tooltip.classed('active', true)
                        .html( function () {
                          if (d['number_unvacc'] < 1000000){
                            var rounded = Math.round(d['number_unvacc']/1000);
                            var unvaccinated = fmtComma(rounded * 1000);
                            if (rounded < 1) {
                                unvaccinated = '>1,000';
                            }
                          } else {
                            var rounded = (d['number_unvacc']/1000000).toFixed(1);
                            var unvaccinated = rounded + ' million';

                          }
                          return '<strong>'+ d['name'] + ': </strong><br>' + unvaccinated + ' (' + fmtComma( (100 - d['2016'])) + '%) unvaccinated';
                        })
        ...
        scaleKey.forEach(function(d, i) {
            ...
            scaleDots.append('text')
                ...
                .text(function() {
                    var amt = d;
                    var label = Math.round((amt/1000000));
                    return label + 'M';
