...
var renderGroupedStackedColumnChart = function(config) {
    ...
    var colorScale = d3.scale.ordinal()
        ...
        .range([ COLORS['blue1'], COLORS['blue3'] ]);
    ...
    /*
     * Render axes to chart. (the bold labels)
     */
    ...
    xScale.domain().forEach(function(c, k) {
        ...
        var columnLabels = categoryElement.selectAll('.x.axis.bars text')
            .attr('y', 15)
            .attr('dy', 0)
            .call(wrapText, xScaleBars.rangeBand(), 13);
        ...
        bars.selectAll('text')
            ...
            .enter().append('text')
                ...
                .attr('y', function(d) {
                    ...
                    if (textHeight + valueGap * 2 > barHeight && d['val'] == 15) {
                        d3.select(this).classed('raised', true);
                        return centerPos - barHeight - 2;
                    } else if (textHeight + valueGap * 2 > barHeight) {
    ...
    // shift category labels
    var barLabels = chartElement.selectAll('.x.axis.bars text');
    var maxBarLabelHeight = 0;
    barLabels[0].forEach(function(d,i) {
        var t = d3.select(d);
        var thisLabelHeight = t.node().getBBox().height;
        if (thisLabelHeight > maxBarLabelHeight) {
            maxBarLabelHeight = thisLabelHeight;
        }
    });

    chartElement.selectAll('.x.axis.category text').attr('y', 20 + maxBarLabelHeight);
    chartElement.selectAll('.x.axis.category tspan').attr('y', 20 + maxBarLabelHeight);
