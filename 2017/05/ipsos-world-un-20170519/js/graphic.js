...
var render = function(containerWidth) {
    ...
    charts.forEach(function(d,i) {
        ...
        // "Correct answer" and "your answer" labels
        // using "i == 0" as a temporary proxy for a "your answer" var passed in
        var annotText = '&nbsp;';
        // if (isCorrect && i == 1) {
        //     annotText = '<span class="correct">Correct answer</span> | Your answer';
        // } else if (isCorrect) {
        if (isCorrect) {
            annotText = '<span class="correct">Correct answer</span>';
        // } else if (i == 1) {
        //     annotText = 'Your answer';
        }
        var hdr = chartElement.append('div')
            .attr('class', 'hdr');
        var annotHeader = hdr.append('h4')
            .html(annotText);
        var header = annotHeader.append('h3')
            .text(LABELS['hed_' + d]);
        if (showLabels) {
            hdr.attr('style', 'margin-left:' + (labelWidth + labelGap) + 'px;');
        }
