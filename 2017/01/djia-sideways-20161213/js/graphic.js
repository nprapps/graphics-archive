...
var renderLineChart = function(config) {
    ...
    var aspectWidth = 9;
    var aspectHeight = 16;
    // var aspectWidth = isMobile ? 9 : 3;
    // var aspectHeight = isMobile ? 16 : 4;
    var minHeight = 500;
    ...
    var xAxisTop = d3.svg.axis()
        ...
        .tickFormat(function(d,i) {
            if (isMobile && d > 0) {
                ...
                if ((config['width'] <= 600 && i % 2 == 0) || (config['width'] > 600)) {
                    return fmtComma(d);
                }
            }
        });
    ...
    var recession = chartElement.append('g')
        ...
        .enter()
            .append('rect')
                ...
                .attr('width', chartWidth + margins['right'])
                .attr('y', function(d) {
                    return yScale(d['begin']);
                })
        ...
        .enter().append('div')
            ...
            .html(function(d) {
                if (isMobile) {
                    ...
                    return '<strong>' + formatFullDate(d[dateColumn]) + ':</strong> ' + d['annotations'];
                }
            })
    ...
    // recession label
    annotatedText.selectAll('.recession')
        .data(recession_dates)
        .enter().append('div')
            .attr('class', function(d) {
                return 'value recession ' + classify(formatFullDate(d['begin']));
            })
            .html('Economic recession')
            .attr('style', function(d) {
                var topPos = yScale(d['begin']) + 6;
                if (isMobile) {
                    topPos -= 3;
                }
    ...
    _.each(annotations, function(d, i) {
        ...
        if (!isSpecial) {
            ...
            thisPoint.append('line')
                .attr('x1', xScale(d['Dow Jones Industrial Average']))
            ...
            var thisStartX = xScale(d['Dow Jones Industrial Average']);
            var thisStartY = yScale(d['date']).toFixed(0);
            var thisEndX = chartWidth + 3;
        ...
        thisPoint.append('circle')
            .attr('cx', xScale(d['Dow Jones Industrial Average']))
