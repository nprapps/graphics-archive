...
var render = function(containerWidth) {
    ...
    var colorScale = d3.scale.ordinal()
        ...
        .range([ COLORS['orange3'], COLORS['orange5'], '#ccc', COLORS['teal5'], COLORS['teal3']]);
    ...
    var navElement = containerElement.append('ul')
        .attr('class', 'nav');
    ...
    charts.forEach(function(d,i) {
        ...
        containerElement.append('div')
            .attr('class', 'chart chart-' + i + ' year-' + d);
    ...
    // init buttons
    var navButton = navElement.selectAll('li')
        .on('click', function() {
            // turn everything active off
            d3.select('.chart.active').classed('active', false);
            navButton.classed('active', false);

            // figure out what button/year this is
            var btn = d3.select(this);
            var btnYear = btn.attr('class');
            btn.classed('active', true);

            // turn charts on/off accordingly
            d3.selectAll('.chart')
                .classed('active', function() {
                    if (d3.select(this).classed(btnYear)) {
                        return true;
                    } else {
                        return false;
                    }
        ...
        // init charts. only the first one (i == 0) should be visible.
        var renderedCharts = d3.selectAll('.chart')
        ...
        navButton.classed('active', function(d,i) {
                if (i == 0) {
                    return true;
                } else {
                    return false;
