...
var renderLocatorMap = function(config) {
    ...
    if (DATA_DISPLAY == 'choropleth') {
        if (DATA_POSITIONING == 'country') {
            ...
            var colorRange = [ COLORS['blue6'], COLORS['blue5'], COLORS['blue4'], COLORS['blue3'], COLORS['blue2'], COLORS['blue1'] ];
            var colorNoData = '#EEE';
            //var colorBinsCount = colorBins.length;
            var colorBins = [1000, 10000, 100000, 500000, 1000000, 3000000];
            var binLabels = ['1,000', '10,000', '100,000', '500,000', '1 million'];
            var colorScale = d3.scale.threshold()
                .domain(colorBins)
                .range(colorRange);

            // Render legend
            var legendWrapper = containerElement.insert('div', ':first-child')
                .attr('class', 'key-wrap numeric-scale');

            legendWrapper.append('h3')
                .text('Refugee population, mid-2016');

            var legend = legendWrapper.insert('ul')
                .attr('class', 'key');
