...
var renderLineChart = function(config) {
  ...
  var aspectWidth = isMobile.matches ? 4 : 8;
  var aspectHeight = isMobile.matches ? 3 : 2;
  ...
  var dates2 = config.data[1].values.map(d => d.date);
  ...
  chartElement.append('text')
    .classed('chart-label', true)
    .attr('x', function(){
      var dates = recession_dates[recession_dates.length - 1];
      return xScale(dates.begin) + ((xScale(dates.end) - xScale(dates.begin)) / 2);
    })
    .attr('y', isMobile.matches ? chartHeight - 15 : 20)
    .attr('y', isMobile.matches ? -20 : 20)
