...
var renderLineChart = function(config) {
  ...
  chartElement
    ...
    .append("text")
    .attr("x", function(d, i) {
      if (d[dateColumn].getFullYear() == '2020') {
        return xScale(d[dateColumn]) + 5;
      }
      if (isMobile.matches) {
        return xScale(d[dateColumn]) - 60;
      }
      return xScale(d[dateColumn]) - 80;
      
    })
    .attr("y", function(d) {
      return yScale(d[valueColumn]) + 5;
    })
    .text(function(d) {
      ...
      var label = `Q${q} ${d.date.getFullYear()}: $${fmtComma(value)}`;
      return label;
    })
    .call(wrapText, margins.right, labelLineHeight);
};
