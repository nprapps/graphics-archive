...
var order_dates = [
  {
    begin: new Date(2020, 2, 22),
    text: 'New York issues “stay-at-home” order',
  },
];
...
var renderLineChart = function (config) {
  ...
  var yScale = d3.scaleLinear().domain([0, 0.65]).range([chartHeight, 0]);

  var colorScale = d3
    .scaleOrdinal()
    .domain(config.data.map(d => d.name))
    .range(['#ddd', COLORS.teal3]);
  ...
  annotation
    ...
    .html(function (d) {
      ...
      var value = ': ' + (d[valueColumn] * 100).toFixed(0) + '%';
      return text + ' ' + value;
    })
