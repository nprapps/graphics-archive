...
module.exports = function(config) {
  ...
  var xAxis = d3
    ...
    .tickFormat(function(d, i) {
      if (isMobile.matches) {
        ...
        var q = d.getMonth() / 3 + 1;
        if (q == 1) return d.getFullYear(); //return `Q${q} ${d.getFullYear()}`;
        return "";
      }
    });
  ...
  // added shaded regions
  var start = new Date(2007, 11, 1);
  var end = new Date(2009, 5, 1);
  var recession = chartElement.append("g")
    .attr("class", "recession")
  
  recession.append("rect")
    .attr("x", xScale(start))
    .attr("y", 0)
