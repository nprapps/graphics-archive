...
var renderMap = function(config) {
  ...
  var projection = d3.geoRobinson()
    ...
    .scale(config.width / 1.9 / Math.PI)
    .center([15, 20]);
  ...
  chartElement.selectAll(".countries")
      .data(config.data.sort(function(a, b) {
        var aProp = a[deathProperty] ? a[deathProperty] : 0; 
        var bProp = b[deathProperty] ? b[deathProperty] : 0; 
        return aProp - bProp;
      }))
      .enter().append("path")
      .attr("class", d => d[deathProperty] ? "country on" : "country")
      .attr("d", path);
  ...
  // Render infection bubbles and set up mouseover tooltip.
  chartElement.selectAll("circle.bubble")
    ...
    .enter()
    .append("circle")
    .attr("class","bubble deaths")
    .attr("r",function(d){    
      if (d[deathProperty] && d[deathProperty] > 0) { 
        return radius(d[deathProperty]);        
      }
      // Add me back in to display a circle even for unaffected countries.
      // else {
      //   return radius(1);
      // }
    })
  ...
  textGroups
    ...
    .attr("class","label main")    
    .attr("dy", 5)
    .attr("dx", d => (radius(d[deathProperty]) + d.label_offsetX)*labelSide(d.labelSide))
  ...
  textGroups
    .append("line")
    .attr("class","annoLine")
    .attr("x1",d => (radius(d[deathProperty]) + d.label_offsetX - 3 )*labelSide(d.labelSide))
    .attr("x2",d => (radius(d[deathProperty]) + 3)*labelSide(d.labelSide))
    .attr("y1",0)
    .attr("y2",0);
...
function getCenter(d, projection) {
  ...
  if (d[nameProperty] == "United States") {
    center[1] = center[1] + chartHeight * .03;
  }
  if (d[nameProperty] == "Canada") {
    center[0] = center[0] - chartWidth * .03;
  }
  if (d[nameProperty] == "France") {
    center[0] = center[0] + chartWidth * .02;
    center[1] = center[1] - chartWidth * .01;
  }
