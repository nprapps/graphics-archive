      ...
      if (!skipLabels.includes(key) && !!d[key]) {
        ...
        if (hasAnnotation) {
          ...
          annotations.push({
            date: d.date,
            amt: d[key],
            amtWinner: d.annotate_party == "Rep" ? d.Rep : d.Dem,
            amtSecond: d.annotate_party == "Rep" ? d.Dem : d.Rep,
            annotate_party: d.annotate_party,
            series: key,
            xOffset: xOffset,
            yOffset: yOffset,
            align: d.align,
...
var renderLineChart = function (config) {
  ...
  var yScale = d3.scaleLinear().domain([min, 82000000]).range([chartHeight, 0]);
  ...
  var yAxis = d3
    ...
    .tickFormat(function (d, i) {
      ...
      return d / 1000000 + (isMobile.matches ? " M" : " million");
    });
  ...
  annotation
    .append("text")
    .html(function (d) {
      if (!(d.annotate_party == d.series)) return;
      var hasCustomLabel = d.label != null && d.label.length > 0;
      var text = hasCustomLabel ? d.label : fmtYearFull(d[dateColumn]);
      return text;
    })
  ...
  annotation
    ...
    .html(function (d) {
      if (!(d.annotate_party == d.series)) return;
      var valueRep = (d.annotate_party == "Dem" ? "Dem: " : "Rep: ") + (d.amtWinner/1000000).toFixed(1) + 'M';
      return valueRep;
    })
    .attr("x", d => xScale(d[dateColumn]) + d.xOffset + annotationXOffset)
    .attr("y", d => yScale(d[valueColumn]) + d.yOffset + annotationYOffset + 15)
    .attr("class", d => d.annotate_party == "Rep" ? "rep" : "dem")
    .style("text-align", d => d.align)
    .call(wrapText, annotationWidth, annotationLineHeight);
  ...
  annotation
    ...
    .html(function (d) {
      if (!(d.annotate_party == d.series)) return;
      var valueDem = (d.annotate_party == "Dem" ? "Rep: " : "Dem: ") + (d.amtSecond/1000000).toFixed(1) + 'M';
      return valueDem;
    })
    .attr("x", d => xScale(d[dateColumn]) + d.xOffset + annotationXOffset)
    .attr("y", d => yScale(d[valueColumn]) + d.yOffset + annotationYOffset + 30)
    .attr("class", d => (d.annotate_party == "Rep" ? "dem" : "rep"))
    .style("text-align", d => d.align)
    .call(wrapText, annotationWidth, annotationLineHeight);
};
