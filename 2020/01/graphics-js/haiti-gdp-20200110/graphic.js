...
var renderColumnChart = function(config) {
  ...
  var xAxis = d3
    ...
    .tickFormat(function(d, i) {
      var thisDate = new Date(d, 0, 1);

      if (isMobile.matches) {
        return "\u2019" + fmtYearAbbrev(thisDate);
      } else {
        return fmtYearFull(thisDate);
      }
    });
  ...
  chartElement
    ...
    .attr("class", function(d) {
      var polarity = d[valueColumn] < 0 ? ' negative' : '';
  ...
  chartElement
    ...
    .append("text")
    .text(function(d) {
      var polarity = d[valueColumn] > 0 ? '+' : '';
      if (isMobile.matches) {
        return polarity + d[valueColumn].toFixed(1);
      } else {
        return polarity + d[valueColumn].toFixed(1) + '%';
      }
    })
