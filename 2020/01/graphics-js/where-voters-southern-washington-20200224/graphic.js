...
var renderLineChart = function(config) {
  ...
  chartElement
    ...
    .text(function(d) {
      ...
      if (!isMobile.matches && value == 53.33) {
        label = d.name + ": +" + label + "%";
      } else if (!isMobile.matches && value !== 53.33) {
        label = d.name + ": ";
      } else if (isMobile.matches && value !== -33.33) {
        label = "+" + label + "%";
      } else if (isMobile.matches && value == -33.33) {
        label = label + "%";
      }
    ...
    .append("text")
    .attr("x", d => xScale(lastItem(d)[dateColumn]) + 5)
    .attr("y", d => yScale(lastItem(d)[valueColumn]) + 17)
    .text(function(d) {
      var item = lastItem(d);
      var value = item[valueColumn];
      var label = value.toFixed(1);

      if (!isMobile.matches && value == 42.86) {
        return "+" + label + "%";
      }
      else if (!isMobile.matches && value == -33.33) {
        return + label + "%";
