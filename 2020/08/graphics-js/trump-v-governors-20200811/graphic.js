...
var pymChild;
var skipLabels = ["date", "date_label", "annotate", "icon", "action", "x_offset", "y_offset", "rightOffset"];
...
var formatData = function(data) {
  ...
  data.forEach(function(d) {
    ...
    for (var series in d) {
      ...
      // Annotations
      var hasAnnotation = !!d.annotate || !!d.action;
      if (hasAnnotation) {
        var hasCustomLabel = d.annotate !== true;
        var label = hasCustomLabel ? d.annotate : null;
        var icon = hasCustomLabel ? d.icon : null;
        var dateLabel = hasCustomLabel ? d.date_label : null;

        var hasActionLabel = d.action !== true;
        var action = hasActionLabel ? d.action : null;

        var xOffset = Number(d.x_offset) || 0;
        var yOffset = Number(d.y_offset) || 0;

        var rightOffset = Number(d.rightOffset) || 0;
