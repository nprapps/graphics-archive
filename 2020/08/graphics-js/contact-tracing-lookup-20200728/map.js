...
var formatData = function() {
  if (!LABELS.show_territories) {
    ...
    DATA = DATA.filter(d => territories.indexOf(d.label) == -1);

    DATA = DATA.map(function(d) {
      var categories = LABELS.legend_labels.split("|").map(l => l.trim());
      if (d.staff_public.toLowerCase() == "yes" && d.data_public.toLowerCase() == "yes") {
        d.public = categories[2];
        d.link = d.staff_link.split(" ").map(l => l.trim()).filter(l => l.match(/gov|http/))[0];
      } else if (d.staff_public.toLowerCase() == "yes") {
        d.public = categories[0];
        d.link = d.staff_link.split(" ").map(l => l.trim()).filter(l => l.match(/gov|http/))[0]
      } else if (d.data_public.toLowerCase() == "yes") {
        d.public = categories[1];
        d.link = d.data_link.split(" ").map(l => l.trim()).filter(l => l.match(/gov|http/))[0]
      } else {
        d.link = null;
        d.public = categories[3];
      }
      return d;
    });
  }
...
var renderStateGridMap = function(config) {
  ...
  config.data.forEach(function(state) {
    ...
    if (state[valueColumn] !== null) {
      ...
      var stateClass = "state-" + classify(state.label);
      ...
      chartElement
        ...
        .attr("fill", function(d){
          ...
          if (state[valueColumn] == 0) {
            var color = "#ccc";
          } else {
            var color = colorScale(state[valueColumn])
          } return color;
        })
  ...
  chartElement
    ...
    .text(function(d) {
      var state = STATES.filter(s => s.name == d.label).pop();
    ...
    .attr("x", function(d) {
      var className = `.state-${classify(d.label)}`;
