...
var renderLineChart = function(config) {
  ...
  var confidence = [].concat(
    ...
    DATA.map(d => ({ date: d.date, value: d.upper })).reverse()
  );
  var cLine = d3.line().x(d => xScale(d[dateColumn])).y(d => yScale(d.value));
  ...
  for (i in DATA) {
    if (DATA[i].death_mean > maxItem.death_mean) {
      maxItem = DATA[i]
    }
  }

  console.log(maxItem)
  var annotText = "Peak deaths: " + months[maxItem[dateColumn].getMonth()] + " " + maxItem[dateColumn].getDate() + " Estimate: " + fmtComma(parseInt(maxItem.death_mean.toFixed(0))) 
  ...
  chartElement
    ...
    .attr("y", d => yScale(maxItem['death_mean']) - 19)
    .text(annotText)
    .call(t => wrapText(t, 155,13));
