...
var render = function(containerWidth) {
  ...
  for (var i = 0; i < chartList.length; i++) {
    ...
    .range([ COLORS.yellow5, COLORS.teal5, COLORS.orange5, COLORS.red3 ]);
    ...
    renderLineChart({
      ...
      annotations: annotations.filter(function(d){
        if (d.series == chartList[i].primary){
          return d;
        } else if (d.series == "overall" && d.label && chartList[i].primary == goesFirst) {
          return d;
        }
      }),
      marginLeft,
...
var renderLineChart = function(config) {
  ...
  annotation.append("text")
    ...
    .html(function(d) {
      ...
      if (hasCustomLabel && d.series == "Asian") {
    ...
    .attr("y", function(d, i){
      ...
      var height_i = i < 3 ? -12 : 0;
      
      return yScale(d[valueColumn]) + (d.yOffset + annoConfig.yOffset)*sign + height_i;
    })
