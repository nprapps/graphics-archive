...
var render = function() {
  ...
  ALLSTATES = TABLE.filter(d => !d.old).map(d => d.label).sort((a,b) => a > b ? 1 : -1 );
...
var renderDrop = function(userData, container, width) {
    ...
    if (userData != "Select a state") {
        ...
        for (i in matchUserDataRows) {
          ...
          d['footnote6'] = matchRow.footnote_6 || '';
          d['footnote7'] = matchRow.footnote_7 || '';
...
var renderAutoText = function(userDataMeasures) {
  ...
  d3.select(".footnote-6").html('');
  d3.select(".footnote-7").html('');
  ...
  if (stateName != "Select a state") {
    ...
    for (i in userDataMeasures) {
      ...
      // .style("margin-bottom", 0)
        ...
        d3.select(".footnote-5").append("p")
        .html(`
          ${stateName.footnote5}
        `);
        d3.select(".footnote-6").append("p")
        .html(`
          ${stateName.footnote6}
        `);
        d3.select(".footnote-7").append("p")
        .html(`
          ${stateName.footnote7}
        `);
...
// <pattern id="reserve" width="4" height="4"
//             patternUnits="userSpaceOnUse"
//             patternTransform="rotate(-45)">
//   <path d="M5,0L5,5"
//         style="stroke:#8BC0BF; stroke-width:9" />
// </pattern>
  ...
  group
    ...
    .attr("fill", function(d) {
      console.log(d)
      if (config.data[0].old) {
        if (d.name == 'Current staffing level per 100K residents'){
          return 'url(#reserve)'
        }
        return 'url(#sufficient)'
      }
  ...
  if (min < 0) {
    ...
    .attr("text-anchor", function(d){
      if (config.data[0]['needed per cap'] + 150 > max) {
        return "end"
      }
      return "start"
    })
