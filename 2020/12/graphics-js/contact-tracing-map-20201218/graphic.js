...
var renderStateGridMap = function(config) {
  ...
  var keyItem = legendElement.append("li").classed("key-item special", true);

  keyItem.append("b");
  keyItem.append("label").text("Did not respond. Estimate based off of October survey data");
  ...
  config.data.forEach(function(state) {
    if (state[valueColumn] !== null) {
      ...
      chartElement.select('.states')
        .select("." + stateClass)
        .attr("class", stateClass + " state-active " + categoryClass)
        .attr("fill", function(d){
          if (state.old) {
            if (state[valueColumn] == "Does not meet estimated need") {
              return "url(#insufficient)"
            } else if (state[valueColumn] == "Has reserve staff to meet estimated need") {
              return "url(#reserve)"
            }
            return "url(#sufficient)"
          }
          else if (state[valueColumn] == 0) {
            var color = "#ccc";
          } else {
