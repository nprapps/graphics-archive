...
var renderStackedBarChart = function(config) {
  ...
  var colorScale = d3
    ...
    .range([COLORS.red2, COLORS.red4, COLORS.red6, "#ccc"]);
  ...
  chartWrapper
    ...
    // make appear as units

    d3.selectAll(".graphic-wrapper .group")
      .attr('', function(d,i) {
        values = d.values
        console.log(values)
        var totalStaffers = 0
        for (i=0; i<values.length; i++) {
          totalStaffers = totalStaffers + values[i].val
        }
        var unitWidth = xScale(totalStaffers)/totalStaffers;
        var currentPosition = 0
        for (i=0; i<totalStaffers; i++) {
          console.log(i)
          d3.select(this).append("line")
            .attr("x1", currentPosition + unitWidth)
            .attr("x2", currentPosition + unitWidth)
            .attr("y1", 0)
            .attr("y2", barHeight)
            .attr("class", "div-line")
          currentPosition = currentPosition + unitWidth
        }
        
      })

    // d3.selectAll(".graphic-wrapper rect")
    //   .attr("lskdf", function(d,i) {
    //     console.log(d,i)
    //   })
