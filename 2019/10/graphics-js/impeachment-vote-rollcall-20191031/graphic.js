...
var renderStackedBarChart = function(config) {
  ...
  var colorScale = d3
    .scaleOrdinal()
    .domain(["Democrats", "Republicans", "Independents"])
    .range([COLORS.blue3, COLORS.red3, "#888"]);
  ...
  group
    ...
    .filter(d => d.val)
    .text(d => d.val || "")
    .attr("class", d => `${classify(d.name)} ${d.val > 3 ? "left" : "right"}`)
    .attr("x", d => xScale(d.x1))
    .attr("dx", function(d, i, labels) {
      ...
      if (textWidth && textWidth + valueGap * 2 > barWidth) {
        var next = labels[i + 1];
        var end = xScale(d.x0);
        if (!next || next.getAttribute("x") * 1 - end - 10 > textWidth) {
          if (!next) {
            $this.attr("style", `fill: ${colorScale(d.name)}`);
          }
          return 4;
        } else $this.classed("hidden", true);
      }
