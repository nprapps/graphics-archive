...
var formatData = function() {
  DATALIB.forEach(function(d) {d.date = new Date(d['date'], 0, 1);});
  DATAMOD.forEach(function(d) {d.date = new Date(d['date'], 0, 1);});
  DATACONSERV.forEach(function(d) {d.date = new Date(d['date'], 0, 1);});


  // Restructure tabular data for easier charting.
  for (var column in DATALIB[0]) {
    if (column == "date") continue;

    dataSeries1.push({
      name: column,
      values: DATALIB.map(d => ({
        date: d.date,
        amt: d[column]
      }))
    });
  ...
  for (var column in DATAMOD[0]) {
    if (column == "date") continue;

    dataSeries2.push({
      name: column,
      values: DATAMOD.map(d => ({
        date: d.date,
        amt: d[column]
      }))
    });
  ...
  for (var column in DATACONSERV[0]) {
    if (column == "date") continue;

    dataSeries3.push({
      name: column,
      values: DATACONSERV.map(d => ({
        date: d.date,
        amt: d[column]
      }))
    });
