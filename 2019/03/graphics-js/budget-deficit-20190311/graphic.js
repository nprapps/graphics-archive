...
var formatData = function() {
  DATA.forEach(function(d,i) {
    var initialVal = d.label;

    d.label = new Date(Number(initialVal), 0, 1);

    // setting dates for the projected span
    if (initialVal == '2019' || initialVal == '2029') {
      projected_dates.push(d.label);
    }
  });
}
...
var renderColumnChart = function(config) {
  ...
  var xAxis = d3
    ...
    .tickFormat(function(d, i) {
      if (isMobile.matches && i % 3 == 0) {
        return fmtYearAbbrev(d);
      } else if (!isMobile.matches && i % 2 == 0) {
  ...
  var yAxis = d3
    ...
    .tickFormat(function(d) {
      if (d == 0) {
        ...
        return polarity + '$' + Math.abs(val).toFixed(1) + 'T';
      }
    });
   ...
   * Recession bars
   */
  var projected = chartElement.insert('rect','*')
    .attr('class', 'projected')
    .attr('x', xScale(projected_dates[0]))
    .attr('width', (xScale(projected_dates[1]) - xScale(projected_dates[0])) + xScale.bandwidth())
    .attr('y', 0)
    ...
    .enter()
      .append("rect")
        .attr("class", function(d) {
          var c = "bar bar-" + fmtYearFull(d[labelColumn]);
          if (d[valueColumn] > 0) {
            c += ' positive';
          ...
          if (fmtYearFull(d[labelColumn]) >= fmtYearFull(projected_dates[0])) {
            c += ' projected';
          }
          return c;
        })
  ...
  chartElement.append('text')
    ...
    .attr('x', parseInt(chartElement.select('.projected').attr('x')) + (parseInt(chartElement.select('.projected').attr('width')) / 2))
    .attr('y', 20);
};
