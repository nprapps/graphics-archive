...
var renderLineChart = function(config) {
  ...
  var ceilings = values.map(
    v => Math.ceil(v / roundTicksFactor) * (roundTicksFactor + 0.5)
  );
  ...
  chartElement
    ...
    .attr("x", d => {
      // hard-coded annotation position
      return xScale(new Date(2012, 8,1));
    })
    .attr("y", d => yScale(d[valueColumn]) - 20)
    .text(function(d) {
      return d.label;
    })
    .attr("fill", "#666")
