...
var renderBarChart = function(config) {
    ...
    var barLabels = chartElement.append('g')
        ...
        .append('text')
            .html(function(d) {
              ...
              if (label.search('<sup>') > -1){
                label = label.split('<sup>')[0];
              }
                return label;
            })
            ...
            .attr('y', function(d, i) {
                var yOffset = i > splitIndex ? splitHeight : 0;
                var barOffset = 0;
                if (isMobile && d[labelColumn].length > 40) {
                  barOffset = -10;
                }
                return i * (barHeight + barGap) + yOffset + barOffset;
            })
            .attr('dx', function(d) {
                if (d[valueColumn] < 0) {
                    return valueGap;
            ...
            .call(wrapText, (chartWidth / 2) - valueGap, 11);
    ...
    chartElement.append('g')
        ...
        .append('text')
            .text(function(d) {
                return d[valueColumn].toFixed(0) + '% (' + fmtChange(d['dollar_amt']) + ')';
            })
            .attr('x', function(d) {
                return xScale(d[valueColumn]);
            })
...
var fmtChange = function(d) {
  if (d == 0) {
    ...
    return polarity + '$' + Math.abs(d).toFixed(1) + 'B';
  }
}
