...
var renderStackedBarChart = function(config) {
  ...
  var chartHeight = (barHeight + barGap) * config.data.length + largeGap * 2;
  ...
  var group = chartElement
    ...
    .enter()
    .append("g")
    .attr("class", d => "group " + classify(d[labelColumn]).split("-")[1])
    .attr("transform", function(d, i) {
      var grouping = Math.floor(i / 2);
      return (
        "translate(0," + (i * (barHeight + barGap) + largeGap * grouping) + ")"
      );
    });
  ...
  group
    ...
    .enter()
    .append("text")
    .text(d => (d.val ? fmtComma((d.val / 100).toFixed(0) * 100) : null))
    .attr("class", d => classify(d.name))
    ...
    .attr("dx", function(d) {
      ...
      if (textWidth + valueGap * 2 > barWidth || isMobile.matches) {
  ...
  chartWrapper
    ...
    .attr("style", (d, i) =>
      formatStyle({
        ...
        height: barHeight + "px",
        left: "0px",
        top: largeGap * Math.floor(i / 2) + i * (barHeight + barGap) + "px;"
      })
    )
  ...
  d3.selectAll("g.value").each(function(d, i) {
    var total = 0;
      for (key in d) {
        if (!isNaN(d[key])) {
          total = total + d[key];
    ...
    d3.select(this)
      .append('text')
      .attr("x", xScale(total) + 7)
      .attr("dy", barHeight / 2 + 4)
      .attr("class", "total-label")
      .text(fmtComma((total/100).toFixed(0)*100));
