...
var activeView = LABELS.chart1.toLowerCase();
var dataSeries = [];
var pymChild;
...
var render = function() {
  ...
  var navElement = d3.select(container)
    ...
    .attr('class', 'nav')
    .selectAll('button')
    .data([ LABELS.chart1, LABELS.chart2 ])
    .enter()
      .append('button')
        .text(function(d) {
          return d;
...
var onButtonClicked = function(evt) {
  ...
  if (selected === "females") {
    ...
    console.log('here')
    d3.selectAll("[class*='" + selected + "']:not([class*='female'])")
    .classed('active', true)
    .moveToFront();
  }
...
var renderLineChart = function(config) {
  ...
  legend.append("label").text(d => d.name.replace('Males ','').replace('Females ',''));
