...
var formatData = function() {
  ...
  DOTS.forEach(function(d) {
    var y = Number(d.date);
    d.date = new Date(y, 0, 1);
    d.rating_10 = Number(d.rating_10);
    d.rating_50 = Number(d.rating_50);
    d.rating_100 = Number(d.rating_100);
  });
};
...
var renderLineChart = function(config) {
  ...
  legend.append("label").text(d => LABELS['hed_' + d.name]);
  ...
  var dotCats = Object.keys(config.dotData[0])
    .filter(function(d,i) {
      return d != 'date';
    });
    ...
    .enter()
      .append('g')
        .attr('class', function(d) {
          var dFmt = d.replace('_', '-');
          return (classify(dFmt));
        });

  dots.selectAll('circle')
    .data(function(d) {
      var filtered = config.dotData.map(o => {
        var fObj = {};
        fObj['date'] = o['date'];
        fObj['rating'] = o[d];
        fObj['series'] = d;
