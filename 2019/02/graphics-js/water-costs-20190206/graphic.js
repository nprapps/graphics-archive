...
var formatData = function() {
  DATA.forEach(function(d) {
    var thisDate = Number(d.date);
    d.date = new Date(thisDate,0,1);
  });
  ...
  chartElement
    ...
    .enter()
      ...
      .text(function(d) {
        ...
        var last = d.values[d.values.length - 1];
        var val = Number(last[valueColumn].toFixed(0));
        var suffix = "";
        if (d.name == "Cleveland") {
          suffix = " since " + fmtYearFull(first[dateColumn]);
        }
        return d.name + ": $" + fmtComma(val) + " (+" + CHANGE[d.name] + "%" + suffix + ")";
      })
