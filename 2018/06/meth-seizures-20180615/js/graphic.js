...
var render = function(containerWidth) {
    ...
    if (containerWidth <= 180) { // sidebar threshold 
        isSidebar = true;
    } else {
        isSidebar = false;
    }
...
var renderColumnChart = function(config) {
    ...
    var xAxis = d3.svg.axis()
        ...
        .tickFormat(function(d, i) {
            var tick =  '';
            if (isMobile) {
                tick = '\u2019' + fmtYearAbbrev(new Date(d));
            } else {
                tick = fmtYearFull(new Date(d));
            }

            if (fmtYearAbbrev(new Date(d)) == '18') {
                tick = tick + '*';
            }
            return tick;
        });
    ...
    chartElement.append('g')
        ...
        .append('rect')
            ...
            .attr('class', function(d) {
                return 'bar-' + fmtYearAbbrev(new Date(d[labelColumn]));
            });

    chartElement.select('.bars .bar-18').style("fill", COLORS['teal5']);
    ...
    if (!isSidebar) {
        ...
        if (isMobile) {
            chartElement.selectAll('.value text').style('font-size', '9px');
        }
    }
