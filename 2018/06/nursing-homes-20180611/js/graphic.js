...
var formatData = function() {
    var stringKeys = [ 'label', 'label_desc', 'category' ];
    var nonStateKeys = [ 'national_avg', 'max', 'min', 'scale_min', 'scale_max' ];
    ...
    DATA.forEach(function(d) {
        ...
        if (typeof stateValues[d['category']] == 'undefined') {
            stateValues[d['category']] = [];
        }
...
var render = function(containerWidth) {
    ...
    var selectorStates = stateValues[categories[0]][0].map(function(d) {
      return {
        state: d['state'],
    ...
    initStateSelector(
        stateSelector,
        selectorStates,
        // Default to California, since this is one of the states highlighted in the story
        'CA'
    );
...
var renderDotChart = function(config) {
    ...
    var labelDescColumn = 'label_desc';
    ...
    var barGap = 5;
    var barShift = 0;
    var stateDotRadius = 6;
    // var labelWidth = 150;
    // var labelMargin = 15;
    var labelWidth = 0;
    var labelMargin = stateDotRadius + 5;
    ...
    containerElement.append('h5')
        .html(config['data'][0][labelDescColumn]);
     ...
     */
    var min = config['data'][0]['scale_min'];
    var max = config['data'][0]['scale_max'];
    ...
    var stateDots = chartElement.append('g')
        ...
        .enter().append('g')
            .attr('class', function(d,i) {
                return 'dots dots-' + i + ' ' + classify(config['data'][i][labelColumn]);
            })
    ...
    chartElement.select('.dots-0')
        .append('text')
        .text(LABELS['annot_national_avg'] + ': ' + config['data'][0]['national_avg'] + '%')
        .attr('class', 'average')
     ...
     * Render bar labels.
     */
    // var rowLabels = chartWrapper.append('ul')
    //     .attr('class', 'labels')
    //     .attr('style', function() {
    //         if (isMobile) {
    //             return formatStyle({
    ...
    //         })
    //         .append('span');
    // rowLabels.append('strong')
    //     .html(function(d) {
    //         return d[labelColumn];
    //     });
    // rowLabels.append('i')
    //     .html(function(d) {
    //         return d[labelDescColumn];
    //     });
}
    ...
    if (d3.select(this).property('value') != LABELS['select_one']) {
        ...
        var dots = d3.selectAll('circle.' + selected)
            ...
            .classed('active', function(d, i) {
                ...
                var val = d['value'].toFixed(0) + '%';
                // if (i == 0) {
                    val = statePostalToAP(d['state']) + ': ' + val;
                // }

                var dotLabelClasses = 'state-value ' + classify(d['state']);
