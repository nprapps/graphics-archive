...
var renderLineChart = function(config) {
    ...
    // recession label
    if (config['id'] == 0) {
        var recessionBar = chartElement.select('.recession rect:nth-child(2)');
        var recessionBarX = +recessionBar.attr('x');
        var recessionBarWidth = +recessionBar.attr('width');

        chartElement.append('text')
            .attr('class', 'recession label')
            .attr('x', (recessionBarX + (recessionBarWidth / 2)))
            .attr('y', 20)
            .text('Recession');
    }
}
