...
var render = function(containerWidth) {
    ...
    if (containerWidth <= MOBILE_THRESHOLD) {
        isMiddle = false;
        isMobile = true;
    } else if (containerWidth <= MIDDLE_THRESHOLD && containerWidth > MOBILE_THRESHOLD) {
        isMiddle = true;
        isMobile = false;
        numCols = 2;
    } else {
...
var renderLineChart = function(config) {
    ...
    if (config['showLabels']) {
        if (!isMobile) {
            ...
            margins['right'] += config['labelWidth'];
        }
    }
    ...
    chartElement.append('g')
        ...
        .enter().append('text')
            ...
            .text(function(d) {
                ...
                var label = last[valueColumn].toFixed(0) + '%' + change;
                return label;
            });
}
