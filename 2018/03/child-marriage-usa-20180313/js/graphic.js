...
var renderStateGridMap = function(config) {
    ...
    if (LABELS['legend_labels'] && LABELS['legend_labels'] !== '') {
        ...
        temp = categories[categories.length-1]
        categories.pop();
    ...
    if (config['isNumeric']) {
        ...
        var colorScale = d3.scale.ordinal()
            .domain(categories)
            .range(['#ea8259', '#C4E1DE', '#7DC9D0', '#0DA0AD', '#179193', '#175E60', '#002B2D']);
            //.range(['#ea8259', COLORS['teal6'], COLORS['teal5'], COLORS['teal4'], COLORS['teal3'], COLORS['teal2'], COLORS['teal1']]);
            //.range(['#ea8259', '#f0f9e8', '#ccebc5', '#a8ddb5', '#7bccc4', '#43a2ca', '#0868ac']);
            //range(['#ea8259', '#c6dbef', '#9ecae1', '#6baed6', '#4292c6', '#2171b5', '#084594']);
            //.range(['#ea8259', '#cbcfdb', '#c6dbef', '#9ecae1', '#6baed6', '#3182bd', '#08519c']);
            //.range(['#312449', '#315764', '#598088', '#939b96', '#afa99e', '#c9c3b9', '#dcdcdc']);
    }
    ...
    _.each(colorScale.domain(), function(key, i) {
        ...
        keyItem.append('label') // changing legend label for 'na'
            .attr('class', function() { return key; })
            .text(function() {
              if (key == 'na') {
                return 'No age floor';
              } else {
