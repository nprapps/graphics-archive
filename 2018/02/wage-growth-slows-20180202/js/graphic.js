...
var renderLineChart = function(config) {
    ...
    var lastSamePoint = config['data'][0]['values'].filter(function(v) {
            return v[valueColumn] == lastPoint[0][valueColumn];
        }).slice(-2,-1);
    var pointArray = maxPoint.concat(lastSamePoint);
    pointArray = pointArray.concat(lastPoint);
        ...
        .enter().append('text')
            ...
            .text(function(d,i) {
                if (isMobile) {
                    ...
                    var monthLabel = d3.time.format('%B')(d[dateColumn]);
                }
    ...
    valueGroup.selectAll('text')
        .attr('transform', function(d,i) {
            ...
            if (isMobile && i == 1) {
                // Except for second point, to avoid running into the other label on mobile
                return 'translate(0, -' + (textHeight/2) + ')';
            } else {
