...
var renderLineChart = function(config) {
    ...
    highlightArea
        ...
        .text('Drop of 1,175 points on Monday')
    ...
    chartElement.append('g')
        ...
        .enter().append('text')
            ...
            .text(function(d) {
                var last = d['values'][d['values'].length - 1];
                var value = fmtComma(last[valueColumn].toFixed(0));
                var monthFmt = getAPMonth(last[dateColumn]);
                var dayFmt = +d3.time.format('%d')(last[dateColumn]);

                return monthFmt + ' ' + dayFmt + ' ' + d['name'].toLowerCase() + ': ' + value;
            })
            .call(wrapText, margins['right'], isMobile ? 12 : 14);
}
