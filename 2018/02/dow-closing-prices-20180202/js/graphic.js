...
var formatData = function() {
    ...
    for (var column in DATA[0]) {
        if (column == 'date' || column == 'label_axis') {
            continue;
        }
...
var renderLineChart = function(config) {
    ...
    var xAxis = d3.svg.axis()
        ...
        .tickFormat(function(d, i) {
            var monthFmt = getAPMonth(d);
            var dayFmt = +d3.time.format('%d')(d);
    ...
    highlightArea
        ...
        .text('Drop of 1,063 points this week')
    ...
    chartElement.append('g')
        ...
        .enter().append('text')
            ...
            .text(function(d) {
                var last = d['values'][d['values'].length - 1];
                var value = fmtComma(last[valueColumn].toFixed(2));
                var monthFmt = getAPMonth(last[dateColumn]);
                var dayFmt = +d3.time.format('%d')(last[dateColumn]);

                return d['name'] + ' on ' + monthFmt + ' ' + dayFmt + ': ' + value;
            })
            .call(wrapText, margins['right'], 12);
}
