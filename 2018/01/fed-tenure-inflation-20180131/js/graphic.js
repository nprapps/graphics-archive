...
var renderLineChart = function(config) {
    ...
    config['data'].forEach(function(chair) {
        xMins.push(d3.min(chair['values'], function(d) { return d['date'] }));
        xMaxes.push(d3.max(chair['values'], function(d) { return d['date'] }));
    });

    var xScale = d3.time.scale()
        .domain([d3.min(xMins), d3.max(xMaxes)])
        .range([ 0, chartWidth ]);
    ...
    var highlightTextG = highlightG.append('g')
        .attr('transform', 'translate(' + ((highlightX2-highlightX1)/2) + ', 18)')
    ...
    // label the last value on the right side
    chartElement.append('g')
        .attr('class', 'value')
        .selectAll('text')
        .data(config['data'])
