// Global vars
var MOBILE_THRESHOLD = 575;
var pymChild = null;
var isMobile = false;
var questions = ['expectations', 'success'];
...
var formatData = function() {
    SUCCESS_DATA.forEach(function(d) {
        for (key in d) {
            if (key == 'Group') {
                continue;
            }
    ...
    EXPECTATIONS_DATA.forEach(function(d) {
        for (key in d) {
            if (key == 'Group') {
                continue;
            }
...
var render = function(containerWidth) {
    ...
    questions.forEach(function(d,i) {
        var slug = d;
        var thisChartData =  eval(d.toUpperCase() + '_DATA');
        var headline = LABELS['headline_' + slug];

        // Clear existing graphic (for redraw)
        var containerElement = chartWrapper.append('div')
            .attr('id', '#bar-group-' + slug)
            .attr('class', 'question-wrapper');

        containerElement.html('');
        containerElement.append('h1').html(headline).attr('class','h1-' + slug);

        var charts = d3.keys(thisChartData[0]).filter(function(d) {
            return d != 'Group';
        });
        ...
        charts.forEach(function(d,i) {
            ...
            var chartElement = containerElement.append('div')
                .attr('class', 'chart c-' + slug + '-' + classify(d));
            ...
            if (showLabels) {
                header.attr('style', function() {
                    switch (slug) {
                        case 'expectations':
                            return 'padding-left:' + (labelWidth + labelGap) + 'px;' + ' height: 48px;'
                        default:
                            return 'padding-left:' + (labelWidth + labelGap) + 'px;' + ' height: 32px;';
                    }
                });
            }
            ...
            // Render the chart!
            renderBarChart({
                container: '.chart.c-' + slug + '-' + classify(d),
