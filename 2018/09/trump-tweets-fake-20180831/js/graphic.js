...
var pymChild = null;
var isMobile = false;
var skipLabels = [ 'label', 'date', 'values', 'total' ];

var MIDDLE_BREAKPOINT = 600;
...
var formatData = function() {
    DATA.forEach(function(d) {
        ...
        d['date'] = d3.time.format('%m/%Y').parse(d['label']);
...
var renderStackedColumnChart = function(config) {
    ...
    var xAxis = d3.svg.axis()
        ...
        .tickFormat(function(d, i) {
            var thisDate = config['data'][i]['date'];
            var formattedDate = getAPMonth(thisDate);
            if (formattedDate == 'Jan.') {
                formattedDate += ' ' + fmtYearFull(thisDate);
            }
            if ((!isMobile && config['width'] >= MIDDLE_BREAKPOINT) || (!isMobile && config['width'] < MIDDLE_BREAKPOINT && i % 2 == 0) || (isMobile && i % 3 == 0)) {
    ...
    // wrapText on x axis labels
    if (isMobile || config['width'] < MIDDLE_BREAKPOINT) {
        chartElement.selectAll('.x.axis line')
